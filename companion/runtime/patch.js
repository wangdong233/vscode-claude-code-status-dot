...
function restoreExtension(extDir) {
    ...
    const bakSrc = fs.readFileSync(bak, "utf8");
    if (isExtensionPatched(bakSrc)) {
        let clean = stripSeamInPlace(bakSrc);
        if (clean === null)
            clean = stripIifeInPlace(bakSrc);
        if (clean === null || isExtensionPatched(clean)) {
            fail("extension.js.bak is itself patched and cannot be auto-cleaned — refusing to restore. Reinstall the Claude Code extension (it ships a pristine extension.js), then re-run cc-status-dot install.");
        }
...
function reportExtensionPatchHealth(extSrc) {
    ...
    if (hasSeam(extSrc)) {
        ...
        if (ver === INJECT_VERSION && diskHash === wantHash) {
            ...
            log(`  seam prelude: ${ver ?? "pre-v0.1.3"} hash ${diskHash ?? "-"} (STALE — expected ${INJECT_VERSION}:${wantHash}; re-run to re-inject)`);
        }
