...
- 📊 **Bottom SBI 4-light strip** — the status bar's left side (`StatusBarAlignment.Left` + very negative priority `-9999`, near the visible center) renders `🟢done 🟡running 🔵pending 🔴interrupted` side-by-side (v0.1.14 replaces v0.1.13's commandCenter top 4-light, which was unreliable after reload/restart), each light capped 0/1/2/3/N (>=4 shows N), dim ⚪ at 0. 🔵 = awaiting user input (permission/question/elicit, fed by the Notification hook case). Done older than 5 min counts as idle (not green). v0.1.14 simplifies to **a single runtime StatusBarItem** — no CC package.json patch, the IIFE mutates text directly every 500ms; the v0.1.13 setContext→when chain is gone.
