...
/** v0.2.2 — companion VS Code extension (NOT published to Marketplace; shipped
 ...
 *
 *  SBI_DIM_EM is ⚪ (U+26AA, Miscellaneous Symbols) — v0.2.2 reverted the
 ...
 *  see docs/STATES.md §7.5 for the v0.1.17 → v0.2.2 trail (pivot then
 *  revert).
 *
 ...
 *  width gamble; v0.2.2 reverted to ⚪ (commit 55e18b4) because the user
 ...
 *  See docs/STATES.md §7.5 for the full v0.1.17 → v0.2.2 trail. */
...
function buildIIFE(resDir: string): string {
    ...
    // dim/zero emoji (⚪ U+26AA since v0.2.2, which reverted the v0.1.17 ⚪→🟤
    ...
    const bodyLines = [
        ...
        // → "🟢3 🟡1 ⚪0 ⚪0" (v0.1.18 space-separated; ⚪ since v0.2.2
...
// ---------------------------------------------------------------------------
// Companion VS Code extension (v0.2.2)
// ---------------------------------------------------------------------------
 ...
 *
 *  v0.2.2 — known-install-path fallback: macOS users who never ran "Shell
 ...
 *  silently.
 *
 *  v0.2.2 — downgrade guard: BEFORE the `--force` we ask the CLI what version
 ...
 *  SEARCH_DIRS hand-mirrored"): the v0.2.2 companion mirrored INSTALL_DIR /
 ...
 *  or the CLI may be locked). v0.2.2: uses resolveVscodeCli so the uninstall
...
function uninstallCompanion(): void {
    ...
    if (!anyDetected) {
        // v0.2.2: surface this as a warn (not log) so the user notices the
 ...
 *  v0.2.2: surfaces installed-vs-packaged version drift per CLI so a user
...
function reportCompanionStatus(): void {
    ...
    // v0.2.2: also surface the companion-config.json + last-repatch.json files
    // the patcher writes for the companion to read. A missing config means
    // the companion will fall back to its hardcoded constants (v0.2.2
...
function reloadHint(): void {
    // v0.2.2: cross-platform shortcut hint. Cmd+Shift+P on macOS, Ctrl+Shift+P
...
function run(argv: string[]): void {
    ...
    if (args.includes("--patch-only")) {
        // v0.2.2: companion-only entry. Runs ONLY discoverExtension +
    ...
    if (args.includes("--revert")) {
        ...
        const steps: Array<[string, () => void]> = [
            ...
            // v0.2.2: also uninstall the companion .vsix from every detected
    ...
    checkSvgs(RUNTIME_RES_DIR);
    // v0.2.2: also install the companion .vsix into every detected VS Code-
