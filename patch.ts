...
function buildIIFE(resDir: string): string {
    ...
    const bodyLines = [
        ...
        `try{if(!globalThis.__ccsdSidToPanel)globalThis.__ccsdSidToPanel=Object.create(null);if(t.__ccsdSid){globalThis.__ccsdSidToPanel[t.__ccsdSid]=t.panelTab;/*v0.5.18 (race rank 3): register onDidChangeViewState so __ccsdActiveSid is published SYNCHRONOUSLY on focus flip (panelTab.active turns true the instant VSCode switches tabs). Pre-0.5.18 the IIFE never registered this event, so a plain tab switch only updated __ccsdActiveSid via the §H 500ms tick (≤500ms lag) — every consumer still reading __ccsdActiveSid saw a stale sid in that window. This is the WebviewPanel-level activation signal (more dependable than tabGroups.onDidChangeTabs which 'frequently does NOT fire' for webview panels). The listener is auto-cleaned when panelTab disposes.*/try{t.panelTab.onDidChangeViewState(function(e){try{if(e&&e.webviewPanel&&e.webviewPanel.active===true){globalThis.__ccsdActiveSid=t.__ccsdSid;globalThis.__ccsdLastActiveSid=t.__ccsdSid;}}catch(_){}});}catch(_){}}}catch(e){}`,
        ...
        /*v0.5.13/14/16 running→idle decay: since>30min AND tokens.last_ts>30min AND no active subagents (activeSubagents>0 → no decay). v0.5.13 first used last_ts SOLE → false-grayed new-turn active sessions (UserPromptSubmit sets since=now but last_ts is last turn's until the model emits tokens); v0.5.14 made it AND (since=now → AND false → stays yellow). v0.5.16 added the activeSubagents>0 guard because tokens.last_ts does NOT roll up subagent timestamps (cc-status.js readTranscriptIncremental gates ctx.lastTs on !isSub) — a long pure-workflow (main blocked >30min) would otherwise decay despite active subagents; the guard uses the authoritative activeSubagents counter (SubagentStart/Stop inflight). tokens.last_ts is advanced by TOK_EVENTS (PostToolUse/Stop/UserPromptSubmit/PreToolUse/SubagentStop) = real model output. Guard: since + j.tokens.last_ts must exist and be < now (missing → conservatively stay yellow). Symmetric with §H.*/ `else if(st==="running"&&since&&!(j.activeSubagents>0)&&(Date.now()-since)>SBI_RUNNING_STALE_MS&&j.tokens&&j.tokens.last_ts&&(Date.now()-j.tokens.last_ts)>SBI_RUNNING_STALE_MS){st="idle";}`,
        ...
        `var isPend=((j.pending===true)||(__ps&&__ps[files[i].slice(0,-5)]===true))&&st!=="idle";`,
        ...
        /*v0.5.13/14/16 per-tab running→idle decay — SYMMETRIC with §F (since>30min AND tokens.last_ts>30min AND activeSubagents==0; see §F comment for the v0.5.14 AND hotfix + v0.5.16 activeSubagents guard rationale). Replaces v0.5.4 "REMOVED per-tab running decay". Guard: since + j.tokens.last_ts must exist and be < now; missing → conservatively stay yellow.*/ `else if(st==="running"&&since&&!(j.activeSubagents>0)&&(now-since)>SBI_RUNNING_STALE_MS&&j.tokens&&j.tokens.last_ts&&(now-j.tokens.last_ts)>SBI_RUNNING_STALE_MS){st="idle";}`,
