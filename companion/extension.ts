...
//   "onStartup" token which VS Code silently ignored in early v0.2.0 builds
...
const MIN_PATCHER_VERSION = "0.2.0";
 ...
 *  behavior as v0.2.0). Logs nothing on failure; the caller decides whether
 ...
 *  through without a .vsix rebuild), else the v0.2.0 hardcoded list. */
 ...
 *  without reloading the window. v0.2.0 changed from a single boolean to a
 ...
 *
 *  v0.2.0 fix (architecture review, was HIGH): the pre-fix version scanned ALL
...
function discoverCcInThisFlavor(): string | null {
    ...
    // version globally, same as pre-v0.2.0; not ideal for stable+insiders
 ...
 *              the v0.2.0 hardcoded default if config is missing).
 ...
 *
 *  JSDoc correction (v0.2.0 architecture review): the prior comment claimed
 ...
 *  patch reload message with "Later". Closes the v0.2.0 half-state gap:
 ...
 *  patcher that pre-dates the flag write — same v0.2.0 behavior, no
 *  cross-window signal). */
...
export function activate(_ctx: vscode.ExtensionContext): void {
    ...
    // install) we silently fall back to the v0.2.0 hardcoded constants.
