...
function buildIIFE(resDir: string): string {
    ...
    const bodyLines = [
        ...
        /*v0.5.13 (state-machine fix): restore running→idle decay, gated on tokens.last_ts (sole signal) — NOT since, NOT .jsonl mtime. v0.5.4 REMOVED running decay because the then-available signals (.jsonl mtime + __ccsdTranscriptFresh) false-grayed ACTIVE workflows (main transcript idle while subagents ran). tokens.last_ts is strictly better: advanced ONLY by TOK_EVENTS (PostToolUse/Stop/UserPromptSubmit/PreToolUse/SubagentStop) via scanSubagentTranscripts = real model output, AND aggregates subagent tokens into the parent → active workflows (even Workflow-tool agents that don't fire SubagentStart) keep last_ts fresh. This kills stale-running (CC crash/OOM/kill → SessionEnd never fired → file frozen at running; e.g. zombie 90cc10fb: last_ts 12h stale) WITHOUT re-arming the v0.5.4 false-gray. Guard: j.tokens && j.tokens.last_ts must exist and be < now (no last_ts → can't prove stale → conservatively stay yellow, avoid false-gray). Symmetric with §H.*/ `else if(st==="running"&&since&&(Date.now()-since)>SBI_RUNNING_STALE_MS&&j.tokens&&j.tokens.last_ts&&(Date.now()-j.tokens.last_ts)>SBI_RUNNING_STALE_MS){st="idle";}`,
        ...
        /*v0.5.13 (state-machine fix): restore per-tab running→idle decay — SYMMETRIC with §F (tokens.last_ts sole signal + SBI_RUNNING_STALE_MS 30min). Replaces v0.5.4 "REMOVED per-tab running decay" — that removal was to avoid false-graying active workflows; tokens.last_ts aggregates subagent tokens into the parent so active workflows stay fresh (see §F comment for full rationale). Tab + four-light now share ONE decay rule again (v0.5.4 had split them). Guard: j.tokens.last_ts must exist and be < now; no last_ts → conservatively stay yellow (avoid false-gray).*/ `else if(st==="running"&&since&&(now-since)>SBI_RUNNING_STALE_MS&&j.tokens&&j.tokens.last_ts&&(now-j.tokens.last_ts)>SBI_RUNNING_STALE_MS){st="idle";}`,
