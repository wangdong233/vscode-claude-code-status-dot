...
- 📊 **commandCenter top 4-light strip** — title-bar top center renders `🟢done 🟡running 🔵pending 🔴interrupted` side-by-side (v0.1.13 replaces the v0.1.12 right-bottom SBI), each light capped 0/1/2/3/N (>=4 shows N), dim ⚪ at 0. 🔵 = awaiting user input (permission/question/elicit, fed by the new Notification hook case). Done older than 5 min counts as idle (not green). 20 commands + 20 menu items injected into CC's package.json; setContext toggles visibility every 500ms.
