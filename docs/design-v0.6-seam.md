...
**出站（host→webview）**：`user_dialog_request`（consent/refusal 期间 state 与 hasPendingPermissions 均为 false——旧锚 C 唯一等价源）、`tool_permission_request`、`session_states_update`、`session_renamed`（v0.6.9 第 4 值：列表重命名的广播帧 `{sessionId,title}`，是无入站标题流量会话的唯一标题信号；owner-ctx 经 CTX_BY_SID 写 `__ccsdTitle`，§H 画题随之，桥只收已知 sid、无孤儿条目，观察者单写纪律不变）只在 postMessage 方向 → 实例级 defineProperty shadow webview.postMessage：函数体第一行 2-3 次属性读早退（白名单 4 值外原样 `d.value.apply`，热路径 io_message 每 token 一帧不订阅不解析），命中白名单才调 onOutbound 并透传。**信封双形状（v0.6.9）**：CC 2.1.270 的 comm.send 把每条 host→webview 消息包成 `{type:"from-extension",message:{type:"request",…}}`（字节引用：`send($){…this.webview.postMessage({type:"from-extension",message:$})…}`），onOutbound 先解包 from-extension 再终判 `type==="request"`——不解包则 270 上观察器全静默（心跳 `outboundFiltered` 恒 0 实证，v0.6.9 起恢复计数）；pre-270 裸 `{type:"request"}` 与全部测试双形状兼容。**shadow 失败（descriptor 不可配置——实测概率极低）→ 降级为纯入站观察 + 心跳 `degraded.outboundShadow=true`，不引入任何 Proxy**（MED#7）。
