...
**① タブ 5 状態ドット**　各 CC セッションタブの Claude アイコンが状態に応じて変色 —— 🟡 実行中 / 🟢 完了 / 🔴 中断（速ブリンク）/ ⚪ アイドル / 🔵 入力待ち。🔵 入力待ちには 2 種のトリガー：(a) CC が権限承認ダイアログをポップアップするときは CC ネイティブの青ドットに譲る（**上書きしない**）；(b) CC の返信が「確認待ち / let me know / your call」など**意思決定待ち**のセマンティクスを含むとき、タブは自動的に青に転換（running 黄 / done 緑を上書き）——「本当に完了したのか、それとも何か言うのを待っているのか」をタブを見てひと目で判別でき、タブを凝視して推測する必要はない。お気に入り登録済みのセッションタブタイトルには **★** プレフィックス + アイコン下部に金線を付加；アーカイブ済みのセッションタブタイトルには **●** プレフィックス + アイコン下部に灰線を付加（お気に入り / アーカイブは相互排他、同一セッションはどちらか一方のみ）。上部タブバー + 左側「開いているエディター」の両方に表示、両者は同期。

**⚙ バックグラウンドタスク・ギアバッジ**　ステータスドットの真下に小さなグレーのギアが表示される = このセッションには shell 型バックグラウンドタスク(`nohup` 起動の dev server や test watcher など、色の状態では表現できない種類)がまだ実行中であることを示します。タスクがすべて停止するとギアは消え、5 状態の色セマンティクスには影響しません。

**② サイドバー CC お気に入り / CC アーカイブビュー**　Explorer サイドに CC Favorites + CC Archive の 2 つのビューを新設（相互排他：1 つのセッションはどちらか一方にのみ存在）；お気に入りビューはよく使うファイル / セッションを一緒に pin、アーカイブビューは当面使わないセッションを収納。セッションアイコンは open=塗りつぶしバブル / closed=アウトラインバブル、クリックで該当 panel にジャンプまたは新 panel に resume；行内ボタンで相互切替 —— お気に入りビュー [アーカイブ][開く][削除]、アーカイブビュー [お気に入り][開く][削除]、アーカイブ / お気に入りをクリックすると自動的に反対側のビューへ移動。閉じたセッションを右クリックで `claude -r <sid>` コマンドをコピー可能。 両ビューの行は**ドラッグで並べ替え**可能——行の上にドロップするとその手前に挿入、最終行の下にドロップすると末尾へ移動。一度ドラッグするとそのリストは手動順序に切り替わり（新規エントリは引き続き最上部に表示）、変化のないドロップはディスクに書き込みません。
...
**④ ★ お気に入り / ○ アーカイブボタン**　ステータスバーの token 横に 2 つのボタン：★/☆ はワンクリックで現在アクティブな CC セッションをお気に入り追加 / 削除（お気に入り済みは金色の ★、未お気に入りは中空の ☆）；○/● はワンクリックでアーカイブ追加 / 削除（アーカイブ済みは塗りつぶし灰円 ●、未アーカイブは中空の灰環 ○）。お気に入りとアーカイブは相互排他 —— どちらかをクリックすると自動的にもう一方を解除；アクティブな CC セッションがないときは自動的に非表示。
...
### 3. お気に入り / アーカイブ（機能②④に対応）

サイドバー CC Favorites + CC Archive ビュー（相互排他）+ tab ★/● マーク + ステータスバー ★/○ ボタン。アーカイブはお気に入り完全準拠、追加設定不要。
