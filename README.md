...
**⚙ 后台任务齿轮徽标**　状态点正下方多出一颗灰色小齿轮 = 这个会话还有 shell 型后台任务在飞（如 `nohup` 起的 dev server、test watcher——颜色状态体现不了的那类）；悬停 tab 看数量"⚙ N 个后台任务"。任务全停齿轮即消失，不影响五态颜色语义。
...
**④ ★ 收藏 / ○ 归档 按钮**　状态栏 token 旁两个按钮：★/☆ 一键收藏/取消当前活动 CC 会话（已收藏显金色 ★，未收藏显空心 ☆）；○/● 一键归档/取消（已归档显实心灰圆 ●，未归档显空心灰环 ○）。收藏与归档互斥——点其一自动取消另一个；无活动 CC 会话时自动隐藏。
