...
- 📊 **下部 SBI 4 emoji ボール（v0.1.17：単一 SBI + コンパクト連結、0px ギャップ、数字は決してズレない）**——ステータスバー左側（`StatusBarAlignment.Left` + 単一 SBI priority `-9996`、可視中心の近く）に**4 個の emoji ボールが隙間なく並んだ1行**を表示（v0.1.17 は v0.1.16 の 4-SBI 行を単一の SBI に統合し、text は 4 セグメントの連結 `🟢3🟡1⚪0⚪0` になる——根本原因：VSCode の `statusbarpart.css` が SBI ごとに `margin:0 3px;padding:0 5px` を硬埋め（隣接 SBI 間で ~6-16px のギャップ、公開 API では制御不可能；v0.1.16 で見えた「間隔が空いている」のはフレームワークのハード制限で、単一 SBI だけが真のコンパクト性を与える）：🟢完了 / 🟡実行中 / 🔵入力待ち / 🔴中断。各ライトの token は `<ボール><数字>`（ボールは自前の色を持つ）；4 つの token を**区切り文字なしで連結**（**ライト間ギャップ 0px**——行の幅は ~120px から ~70px へ縮小）；カウントは 0/1/2/3/N で頭打ち（>=4 は N 表示）；count>0 → ボール点灯（🟢/🟡/🔵/🔴 プリカラー + 数字が右隣）、count=0 → グレーボール ⚪ +「0」。🔵 = ユーザー入力待ち（permission/question/elicit、Notification hook case 経由）。完了 >5 分は idle 扱い（緑に含まない）。**「数字は決してズレない」は VSCode 自身の CSS 規則 `font-variant-numeric:tabular-nums` によって保証される**（statusbarpart.css がすべてのステータスバー item に tabular-nums を適用——ASCII 数字 0-9 はどんなフォントでも等幅）、emoji フォントのレンダリングからは独立。v0.1.17 は **1 個のランタイム StatusBarItem + 連結 text** を使用（CC の package.json をパッチせず、ThemeColor ブロックなし、IIFE が 500ms ごとに text を直接 mutate）。
