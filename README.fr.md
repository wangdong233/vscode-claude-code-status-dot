...
- 📊 **4 blocs SBI colorés en bas (chiffre à l'intérieur du bloc)** — le côté gauche de la barre d'état (`StatusBarAlignment.Left` + 4 blocs priorité `-9996..-9999`, près du centre visible) affiche 4 **blocs colorés avec le chiffre à l'intérieur** côte à côte (v0.1.15 remplace le format « emoji + chiffre séparé » de v0.1.14) : 🟢terminé (bloc vert) / 🟡en cours (bloc jaune) / 🔵en attente (bloc bleu) / 🔴interrompu (bloc rouge). Chaque bloc plafonné à 0/1/2/3/N (>=4 affiche N) ; quantité>0 → bloc allumé (`statusBarItem.*Background` couleur du thème + chiffre blanc à l'intérieur), quantité=0 → bloc tamisé (fond transparent + « 0 » gris, bloc reste visible). 🔵 = en attente d'entrée utilisateur (permission/question/elicit, alimenté par le cas de hook Notification). Terminé depuis >5 min compte comme idle (pas vert). v0.1.15 utilise **4 instances indépendantes runtime StatusBarItem + 4 couleurs intégrées du thème** (pas de patch du package.json CC, l'IIFE mute text/color/backgroundColor de chaque bloc directement toutes les 500ms) — les couleurs suivent le thème VSCode, stables multiplateforme (plus de dépendance à la police emoji).
