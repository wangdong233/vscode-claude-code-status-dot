...
- 📊 **Tira de 4 luces SBI abajo** — el lado izquierdo de la barra de estado (`StatusBarAlignment.Left` + prioridad muy negativa `-9999`, cerca del centro visible) renderiza `🟢hecho 🟡corriendo 🔵pendiente 🔴interrumpido` una al lado de otra (v0.1.14 reemplaza el commandCenter superior de v0.1.13, que era poco fiable tras reload/restart), cada luz limitada a 0/1/2/3/N (>=4 muestra N), atenuada ⚪ en 0. 🔵 = esperando entrada del usuario (permiso/pregunta/elicit, alimentado por el caso de hook Notification). Hecho de >5 min cuenta como idle (no verde). v0.1.14 simplifica a **un único StatusBarItem en runtime** — sin parchear el package.json de CC, el IIFE muta el texto directamente cada 500ms; la cadena setContext→when de v0.1.13 se eliminó.
