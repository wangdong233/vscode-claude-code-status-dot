...
- **GC 契约**（UserPromptSubmit 扫描，10min throttle）：
  - `<sid>.json`：state=interrupted 永远不删（§7.5）；否则 mtime > `INTERRUPTED_RETENTION_MS`（v0.2.7=7d）或 since > `GC_DRIFT_SINCE_MS`（7d）则 unlink。
