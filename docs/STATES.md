...
**【用户终裁·状态机 v3 语义契约（2026-09-04 两次确认，LOCKED——后续任何版本不得偏移】**：判据是**主会话 UI 上用户是否看到中断信息**。看不到（子 agent 死亡但父会话继续运转自行重试/推进、SDK 内部静默重试）→ 维持黄，"主会话自己控制和修复，用户不用参与"；看得到（主会话回合以错误终止、"API Error" 明确显示、等待人工介入 = 主子都断了）→ 红。红 ⟺ 父 scoped StopFailure 一条路；子事件由主体判别层完全隔离。"部分子死是否红"已被此裁决整体关闭，不得再议。已由 §V3 闸门组机械钉死。

**[v0.6.3 主体判别（状态机 v3 D1）]**：`payload.transcript_path` 含 `/subagents/`（Task 子代理或 Workflow agent，Windows 反斜杠同判）的事件，除 `SubagentStart`/`SubagentStop`（父生命周期事件，父 scoped）外，**一律不触父状态文件**——写、删、token 回填全部跳过。实弹三联：workflow agent 致命 429 的 StopFailure 挂父 sid 把健康/已完结父会话翻红（F1/V9）；child-tp PostToolUse 以 main 身份回退 offset 并重置累计 token（T2）；child SessionEnd 会删父文件。守卫先于 headless 分类器（防 marker 污染）；tp 缺失/未知布局保守按 parent（不回归真父失败）。token 回填额外要求 tp 严格等于父根 `<sid>.jsonl`。`StopFailure` 写入新增 `error_class:'transient'|'fatal'`（FATAL_ERROR_RE 命中才 fatal，未知默认 transient）+ 首错保留（episode 内不覆盖）。`activeSubagents`：UPS/Pre/PostToolUse 的 inflight=null 改为**保留现值**（null≠0——2026-09-04 实测 7 个 workflow agent 在跑而计数恒 0 的 writer 侧根因）；Stop 缺字段仍清零（Stop 是权威）。reader 侧 `__ccsdDecayState` 新增第 4 活性证人 `__ccsdWfAlive`（衰减候选路径才扫 `subagents/workflows/*/agent-*.jsonl` mtime，30s TTL，全失败面 fail-open 回今日行为）——父静默但后台 agent 活跃的会话不再 30min 假 idle。

**第二个 writer-side 删除触发（v0.1.14 R3 文档加固，见 cc-status.js `Bounded GC`；v0.2.7 阈值由 24h 延至 7d）**：`UserPromptSubmit` 在写当前 session 文件之前，会扫描整个 STATE_DIR，**删除**所有 mtime > `INTERRUPTED_RETENTION_MS`（**v0.2.7：7d**）且 `state !== "interrupted"` 的 `.json` 文件——这是跨 session 的全局清理（一个 session X 的 UserPromptSubmit 可能 unlink 另一个 session Y 的陈旧文件）。理由：崩溃 / 被杀的 CC 进程不发 SessionEnd，没有这层全局扫描 `~/.claude/cc-tab-status/` 会无界增长。**[v0.6.4 分型 stayRunning（2026-09-05 用户裁决：主流程已汇报完成等待确认→应绿）]**：`background_tasks[].type` 分型——**shell 型**（nohup 被测 app/dev server 等长驻进程，永不会唤醒会话）不计入 stayRunning，Stop 落 `done`（绿）；**唤醒型**（subagent/workflow/teammate/monitor/cloud session 等，CC 官方语义 'session is paused waiting for background work to wake it back up'）维持 2026-07-13 用户原案的 running（黄）；**未知 type 一律按唤醒型**（fail 向历史黄，绝不假绿）。两裁决由此并存无冲突。同时：①主体判别新增 `agent_id` 判别符（每个 hook payload 基座自带——main loop 为 undefined+main-session，子上下文非空；子上下文 Pre/PostToolUse 携父 session_id+父 transcript_path，路径启发式看不见，此前会无条件写 running 复活黄钉=第 4 复活通道）；②`done` 对后台事件（SubagentStart/任意 SubagentStop）黏滞（镜像 D4 的 interrupted 黏滞——孤儿迟到事件不得翻绿为黄；真续轮由主回路下一事件翻回）。闸门 §V3.9（7 检查）。

**例外**：`state === "interrupted"` 且 **`error_class !== 'transient'`**（v0.6.3 D3：已知致命类或旧文件无该字段读作 fatal）的文件**不删**（即使 >7d）——保留诊断价值；**transient 类（rate_limit/overloaded/429 等可重试错误，含未知串的 fail-open 默认）回归普通 7d mtime 规则**——2026-09-04 两起实弹：可自愈错误被写成 7 天不朽红，与下文 §7.5 "中断态文件不删（保留诊断价值）" 契约一致；聚合层也已经把它们降级为 idle 不计入 🔴。当前 session 文件也跳过（即将被覆写）。**v0.2.7 同步扫描 `.offset` / `.tokens.json` / `.forcereread`**：三者用**纯 mtime 规则**（>7d reap），**不再看 `.json` 是否存在**——SessionEnd 删 `.json` 但保留这三者，旧"按 .json 孤儿判"会立即 reap post-SessionEnd 的 `.offset` 等于没修。`isTokens` 判断必须先于 `isJson`（`.tokens.json` 也 endsWith `.json`），见 §8.7。
...
2. **银灰齿轮，不跟状态色**——状态色语义已锁，齿轮染色会被误读为状态变化；灰 = 中性属性标记。v3.1 初版取 `#4D5157` 中性深灰（N1 对当时**小徽章尺寸**的对抗裁决）；**2026-09-13 用户实测调亮裁决**：状态点尺寸下太暗（深底仅 2.09:1，需仔细观察），参照亮银图样改 **`#C6CCD4`**（深底 10.2:1 一眼可见；白底 1.65:1 与收藏金线白底 1.9:1 同档——亮色属性标记在浅主题下低调有家族先例；比归档灰线亮 2.5×，齿轮/灰线区分更清；阶梯预览 `docs/ladder-*.png` 五档对比后用户拍板）。禁青色、禁状态色。
...
**v0.6.9 记录(2026-09-15):CC 2.1.270 面板会话重命名"闪回"缺陷(上游缺陷,字节级定罪,非本插件)**——CC 扩展 2.1.270(2026-09-13 15:59 安装)对**有活 channel 的会话**从列表重命名必现 `TypeError: J.query.renameSession is not a function`(当日 exthost 日志 4/4 次:lines 53235/53328/53408/53460):宿主 `renameSessionOnCli` 调 `J.query.renameSession(title,sid)`,但 2.1.270 自带 SDK(0.3.270,也是 npm 最新版)的 Query 类全 bundle 13 处 `renameSession` 全在宿主侧、Query 类零 `rename` token——死调用点;npm SDK 仅有模块级 `renameSession()` 函数(sdk.d.ts:2985),CLI 2.1.270 二进制反而有活的 `rename_session` wire handler(source:["remote","host"])。扩展面板从不 spawn PATH CLI(当日 9/9 spawn 均为 `resources/native-binary/claude`,日志 line 34),本地 CLI 2.1.158(落后 112 个 minor)与此无关,**升级 CLI 不治此病**。用户可见=新标题闪现 34-110ms 后回滚(webview 乐观置题+错误响应回滚),但**落盘已成功**——持久化(transcript 追加 `custom-title` JSONL 行)先于抛错的通知步,4/4 次均在;全盘 0 个 `custom-title.json` 属预期:2.1.270 扩展只读该文件不写(CLI 侧产物,重命名走 transcript JSONL 行)。**恢复**:Reload Window 即显示已持久化标题(reader 优先级 customTitle>aiTitle);**规避**:重命名后忽略回滚择机 reload,或先关目标会话面板再改名(无活 channel 不触发,结构性预测未实测);根治等扩展更新。本插件旁观排除:patched 与 .bak 自 97,421 字节起 cmp 全等(+97,420B 纯前置注入),`J.query` 为 bundle 内 SDK 对象不经任何包装面。**附带发现并同版修复**:CC 2.1.270 把全部出站消息包进 `{type:"from-extension",message:…}`(comm.send),旧出站观察器 3 值白名单在 270 上静默失效(心跳 outboundFiltered 恒 0 实证);v0.6.9 起兼容双信封并新增 `session_renamed` 第 4 白名单值(桥标题随列表重命名自愈,观察者单写纪律不变)。
...
## 7.7 v0.5.52 sleep-aware decay + mtime fallback witness

Two reader-side semantics (patch.ts `__ccsdDecayState`, running branch only):

1. **Sleep ledger (unfinished stays yellow across system sleep)**: both 500ms tick bodies open with `__ccsdMarkTick()` — a post-wake tick firing >5s late records a suspension interval (cap 64, per-EH-lifetime). `__ccsdAgeAdj` builds a per-witness adjustment closure (R1 clamp: only suspension overlapping `[ts, now]` is subtracted — sleep predating a witness never shrinks its age) so the running branch measures **awake time**: a mid-turn session frozen by a 3.7h sleep has an effective age of minutes, not hours, and stays 🟡. Failure-mode asymmetry is deliberate: EH stalls/App-Nap/forward clock jumps record phantom suspends = stay-yellow-longer (the safe direction per the user ruling); DarkWake blips leave timers running (no gap recorded). Ledger loss on EH restart falls back to wall clock (pre-v0.5.52 behavior, bounded). R1: the ledger is also RESET on last-panel-out (§Z) — a panel-less dwell (SBI timer cleared, no MarkTick caller) would otherwise record as one unbounded phantom suspend on reopen and compound across open/close cycles.

2. **mtime fallback witness (dead spawns stop counting)**: a running file with NO `tokens.last_ts` (spawn's UserPromptSubmit wrote running, then zero follow-up events — the cc-control SDK test/council spawn class, `entrypoint: claude-vscode`, invisible to the v0.2.2 headless exclusion) previously NEVER decayed (v0.5.13 conservative guard). Now mtime — which witnesses EVERY hook fire — is the fallback witness at **2h** (`SBI_MISSING_LT_STALE_MS`): fresh spawns keep their grace window, 2h-silent single-write spawns flip idle. `!mt` (stat miss) still fails toward decay. The with-last_ts class is byte-identical. Witness ladder: **30m (last_ts) / 2h (mtime fallback) / 24h (as-protection expiry, mtime) / 7d (interrupted + GC)**.

done>5m and interrupted>7d keep WALL clock (deliberate carve-out — no semantic change without user sign-off).
