...
- 📊 **Tira de 4 luces arriba en el commandCenter** — en el centro superior de la barra de título se renderizan `🟢hecho 🟡corriendo 🔵pendiente 🔴interrumpido` una al lado de otra (v0.1.13 reemplaza el SBI inferior derecho de v0.1.12), cada luz limitada a 0/1/2/3/N (>=4 muestra N), atenuada ⚪ en 0. 🔵 = esperando entrada del usuario (permiso/pregunta/elicit, alimentado por el nuevo caso de hook Notification). Hecho de >5 min cuenta como idle (no verde). 20 comandos + 20 items de menú inyectados en el package.json de CC; setContext alterna la visibilidad cada 500ms.
