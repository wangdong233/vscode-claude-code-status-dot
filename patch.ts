...
const I18N_DICT: Record<string, I18NEntry> = {
    ...
    // === v0.3.0 (lane D): tok/s rate + sparkline + chart panel ===
    // qpShowChartLabel / qpShowChartDetail: QuickPick entry that opens the
    //   webview chart panel (form C, pure SVG, CSP strict).
    // ttRateTpl / ttSparklineHint: tooltip rows explaining the live tok/s
    //   suffix + sparkline semantics (NOT used in v0.3.0 — reserved for a
    //   future tooltip-augmentation; key present so tr() never falls back
    //   to the raw key if a future contributor references it).
    // wv* (webview): title, series labels, stat labels for the chart panel.
    qpShowChartLabel: {
        zh: "显示实时速率图表",
        en: "Show live rate chart",
        ja: "リアルタイムレートチャートを表示",
        de: "Live-Raten-Diagramm anzeigen",
        es: "Mostrar gráfico de velocidad en vivo",
        fr: "Afficher le graphique de débit en direct",
        pt: "Mostrar gráfico de taxa ao vivo",
        ru: "Показать график скорости в реальном времени",
    },
    qpShowChartDetail: {
        zh: "打开一个 webview 面板：累计 token 折线 + 瞬时 tok/s 柱图（最近 8 秒）",
        en: "Opens a webview panel: cumulative tokens line + instantaneous tok/s bars (last 8s)",
        ja: "webview パネルを開く：累積トークン折れ線 + 瞬時 tok/s バー（直近 8 秒）",
        de: "Öffnet ein Webview-Panel: kumulative Tokens-Linie + instantaneous tok/s-Balken (letzte 8s)",
        es: "Abre un panel webview: línea de tokens acumulados + barras de tok/s instantáneas (últimos 8s)",
        fr: "Ouvre un panneau webview : ligne des tokens cumulés + barres tok/s instantanées (8 dernières secondes)",
        pt: "Abre um painel webview: linha de tokens acumulados + barras de tok/s instantâneas (últimos 8s)",
        ru: "Открывает панель webview: линия накопленных токенов + столбцы мгновенной скорости tok/s (последние 8 с)",
    },
    ttRateTpl: {
        zh: "瞬时速率：{rate} tok/s（最近 5 秒，不含 cache）",
        en: "Instantaneous rate: {rate} tok/s (last 5s, excludes cache)",
        ja: "瞬時レート：{rate} tok/s（直近 5 秒、cache を除く）",
        de: "Momentanrate: {rate} tok/s (letzte 5s, ohne Cache)",
        es: "Velocidad instantánea: {rate} tok/s (últimos 5s, sin caché)",
        fr: "Débit instantané : {rate} tok/s (5 dernières secondes, cache exclu)",
        pt: "Taxa instantânea: {rate} tok/s (últimos 5s, sem cache)",
        ru: "Мгновенная скорость: {rate} ток/с (последние 5 с, без кэша)",
    },
    ttSparklineHint: {
        zh: "迷你图：最近 8 个采样点的 token 增量（约 4 秒）",
        en: "Mini chart: token delta of last 8 samples (~4s)",
        ja: "ミニチャート：直近 8 サンプルの token 増分（約 4 秒）",
        de: "Mini-Diagramm: Token-Delta der letzten 8 Samples (~4s)",
        es: "Mini gráfico: delta de tokens de las últimas 8 muestras (~4s)",
        fr: "Mini-graphique : delta de tokens des 8 derniers échantillons (~4s)",
        pt: "Mini gráfico: delta de tokens das últimas 8 amostras (~4s)",
        ru: "Мини-график: дельта токенов за последние 8 отсчётов (~4 с)",
    },
    wvChartTitle: {
        zh: "Claude Code token 速率",
        en: "Claude Code token rate",
        ja: "Claude Code トークンレート",
        de: "Claude Code Token-Rate",
        es: "Velocidad de tokens de Claude Code",
        fr: "Débit de tokens Claude Code",
        pt: "Taxa de tokens do Claude Code",
        ru: "Скорость токенов Claude Code",
    },
    wvSeriesCumulative: {
        zh: "累计 token",
        en: "Cumulative tokens",
        ja: "累積トークン",
        de: "Kumulative Tokens",
        es: "Tokens acumulados",
        fr: "Tokens cumulés",
        pt: "Tokens acumulados",
        ru: "Накопленные токены",
    },
    wvSeriesRate: {
        zh: "瞬时速率 (tok/s)",
        en: "Instantaneous rate (tok/s)",
        ja: "瞬時レート (tok/s)",
        de: "Momentanrate (tok/s)",
        es: "Velocidad instantánea (tok/s)",
        fr: "Débit instantané (tok/s)",
        pt: "Taxa instantânea (tok/s)",
        ru: "Мгновенная скорость (tok/с)",
    },
    wvTotalLabel: {
        zh: "累计",
        en: "Cumulative",
...
function buildIIFE(resDir: string): string {
    ...
    const bodyLines = [
        ...
        `var RATE_BUF_CAP=16;var RATE_SPARK_BARS=8;var RATE_WINDOW_MS=5000;var RATE_FLUSH_MS=2000;var RATE_SPARK_CHARS="▁▂▃▄▅▆▇█";if(!globalThis.__ccsdRateBuf)globalThis.__ccsdRateBuf=Object.create(null);if(!globalThis.__ccsdRatePrev)globalThis.__ccsdRatePrev=Object.create(null);if(!globalThis.__ccsdRateMax)globalThis.__ccsdRateMax=Object.create(null);if(!globalThis.__ccsdRateFlush)globalThis.__ccsdRateFlush=Object.create(null);if(!globalThis.__ccsdRateLoaded)globalThis.__ccsdRateLoaded=Object.create(null);function __ccsdRateFromBuf(arr,nowMs){try{if(!arr||arr.length===0)return 0;var cutoff=nowMs-RATE_WINDOW_MS;var sumD=0,oldestTs=nowMs;for(var i=arr.length-1;i>=0;i--){if(arr[i].ts<cutoff)break;sumD+=arr[i].d;if(arr[i].ts<oldestTs)oldestTs=arr[i].ts;}var windowS=(nowMs-oldestTs)/1000;return windowS>0.1?sumD/windowS:0;}catch(_){return 0;}}function __ccsdRateSample(sid,realNow,totalNow,isRunning,nowMs){try{if(!sid)return null;var buf=globalThis.__ccsdRateBuf,prev=globalThis.__ccsdRatePrev,mx=globalThis.__ccsdRateMax;if(!buf[sid]){buf[sid]=[];}if(typeof prev[sid]!=="number")prev[sid]=-1;if(typeof mx[sid]!=="number")mx[sid]=0;var delta=0;if(isRunning&&prev[sid]>=0&&realNow>=prev[sid]){delta=realNow-prev[sid];}prev[sid]=isRunning?realNow:-1;var arr=buf[sid];arr.push({ts:nowMs,d:delta,total:totalNow});while(arr.length>RATE_BUF_CAP)arr.shift();if(delta>mx[sid]){mx[sid]=delta;}else{mx[sid]=mx[sid]*0.85+delta*0.15;}if(mx[sid]<1)mx[sid]=1;var rate=__ccsdRateFromBuf(arr,nowMs);return{rate:rate,max:mx[sid],buf:arr,delta:delta};}catch(_){return null;}}function __ccsdRateSpark(arr,peak){try{if(!arr||arr.length===0||peak<1)return"";var n=arr.length,start=Math.max(0,n-RATE_SPARK_BARS);var out="";for(var i=start;i<n;i++){var v=arr[i].d/peak;var idx=Math.max(0,Math.min(7,Math.floor(v*8)));out+=RATE_SPARK_CHARS.charAt(idx);}while(out.length<RATE_SPARK_BARS&&out.length>0){out=RATE_SPARK_CHARS.charAt(0)+out;}return out;}catch(_){return "";}}function __ccsdRateFlush(sid,arr,mx,nowMs){try{if(!sid||!arr||arr.length===0)return;var fm=globalThis.__ccsdRateFlush;if(!fm[sid])fm[sid]=0;if(nowMs-fm[sid]<RATE_FLUSH_MS)return;fm[sid]=nowMs;var payload={v:1,sid:sid,last_ts:nowMs,recent_max:mx,samples:arr.slice(-600).map(function(s){return{t:s.ts,d:s.d,total:s.total};})};var tmpPath=pth.join(DIR,sid+".rate.tmp");var finalPath=pth.join(DIR,sid+".rate");try{fs.writeFileSync(tmpPath,JSON.stringify(payload));try{fs.renameSync(tmpPath,finalPath);}catch(_){try{fs.unlinkSync(tmpPath);}catch(__){}}}catch(_){}}catch(_){}}function __ccsdRateLoad(sid){try{if(!sid)return null;var ld=globalThis.__ccsdRateLoaded;if(ld[sid])return null;ld[sid]=true;var p=pth.join(DIR,sid+".rate");var raw=fs.readFileSync(p,"utf8");var obj=JSON.parse(raw);if(!obj||obj.sid!==sid||!Array.isArray(obj.samples))return null;var buf=globalThis.__ccsdRateBuf,mx=globalThis.__ccsdRateMax,prev=globalThis.__ccsdRatePrev;buf[sid]=obj.samples.slice(-RATE_BUF_CAP).map(function(s){return{ts:s.t,d:s.d,total:s.total};});mx[sid]=Number(obj.recent_max)||1;if(mx[sid]<1)mx[sid]=1;prev[sid]=-1;return obj;}catch(_){return null;}}/*v0.3.0 (lane D form C): showRateChart opens a webview panel with pure-SVG chart + numeric display, real-time refreshed via postMessage from a per-panel 500ms timer (NOT the shared __ccsdSbiTimer — the chart panel owns its own refresh cadence so closing the panel stops the refresh cleanly). NO external libs (zero deps, zero CSP remote-script risk, ~80 lines of inline JS+SVG vs ~10KB for a charting lib). Strict CSP: default-src 'none', script-src per-call nonce (v0.3.1 hardening; v0.3.0 used unsafe-inline), style-src 'unsafe-inline', img-src data:. Two series: cumulative total (line, monotone-up) + instantaneous tok/s rate (bars, last 16 samples = 8s @ 500ms tick). The tick handler is per-panel (disposed on panel close). Panel is single-instance keyed on globalThis.__ccsdRateChart — re-opening focuses existing.*/function showRateChart(){try{if(globalThis.__ccsdRateChart){try{globalThis.__ccsdRateChart.panel.reveal()}catch(_){}return;}var sid=globalThis.__ccsdActiveSid||globalThis.__ccsdLastActiveSid||"";var panel=vs.window.createWebviewPanel("ccStatusDot.rateChart",tr("wvChartTitle")+" · "+(sid?sid.slice(0,8):"—"),vs.ViewColumn.Active,{enableScripts:true,retainContextWhenHidden:false});globalThis.__ccsdRateChart={panel:panel,sid:sid};var html=__ccsdRateChartHtml(sid);panel.webview.html=html;var msgTimer=setInterval(function(){try{var sid2=globalThis.__ccsdRateChart.sid;var buf=globalThis.__ccsdRateBuf,mx=globalThis.__ccsdRateMax;var arr=buf[sid2]||[];var samples=arr.map(function(s){return{t:s.ts,d:s.d,total:s.total};});var peak=mx[sid2]||1;var j=readTok();var totalNow=(j&&j.tokens)?sumTok(j.tokens.total):0;/*v0.3.0 fix (HIGH): chart-panel timer is READ-ONLY on __ccsdRateBuf. The previous __ccsdRateSample(sid2,0,totalNow,false,Date.now()) call pushed delta=0 samples into the SHARED buffer and clobbered prev[sid2]=-1, which interleaved with the §G SBI tick's real samples to produce zebra-stripe bars + sparkline jitter + EMA peak decay (~15% per chart tick). The §G tick remains the sole writer (1 sample/500ms when running); the chart just re-renders from the buffer via the read-only __ccsdRateFromBuf helper. Buffer growth-rate invariant: 1 sample/500ms regardless of chart open/close.*/var rateNow=__ccsdRateFromBuf(arr,Date.now());panel.webview.postMessage({type:"tick",samples:samples,peak:peak,totalNow:totalNow,rateNow:rateNow,t:Date.now()});}catch(_){}},500);var msgHandler=panel.webview.onDidReceiveMessage(function(m){try{if(m&&m.type==="ready"){panel.webview.postMessage({type:"hello"});}}catch(_){}});panel.onDidDispose(function(){try{clearInterval(msgTimer)}catch(_){}try{msgHandler.dispose()}catch(_){}globalThis.__ccsdRateChart=null;});}catch(e){try{vs.window.showErrorMessage(tr("fbPanelFailPrefix")+(e&&e.message||String(e)))}catch(_){}}}function __ccsdGetNonce(){try{return require("crypto").randomBytes(16).toString("base64")}catch(_){var s="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",r="";for(var i=0;i<32;i++)r+=s.charAt(Math.floor(Math.random()*s.length));return r;}}function __ccsdRateChartHtml(sid){var nonce=__ccsdGetNonce();var title=tr("wvChartTitle");var seriesCum=tr("wvSeriesCumulative");var seriesRate=tr("wvSeriesRate");var totalLbl=tr("wvTotalLabel");var rateLbl=tr("wvRateLabel");var sidLbl=tr("wvSidLabel");var sampleLbl=tr("wvSampleLabel");var sidShort=sid?sid.slice(0,12):"—";var P=[];P.push('<!DOCTYPE html><html><head><meta charset="utf-8">');P.push('<meta http-equiv="Content-Security-Policy" content="default-src \\'none\\'; script-src \\'nonce-'+nonce+'\\'; style-src \\'unsafe-inline\\'; img-src data:;">');P.push("<style>");P.push(__ccsdRateChartCss());P.push("</style></head><body>");P.push('<div class="hdr"><div class="title">'+title+'</div><div class="sid">'+sidLbl+": "+sidShort+"</div></div>");P.push('<div class="stats"><div class="stat"><div class="lbl">'+totalLbl+'</div><div class="val" id="totalVal">—</div></div><div class="stat"><div class="lbl">'+rateLbl+'</div><div class="val" id="rateVal">—</div></div></div>');P.push('<div class="chart"><svg id="svg" width="100%" height="260" viewBox="0 0 800 260" preserveAspectRatio="none"><line class="midline" x1="0" y1="130" x2="800" y2="130" stroke-dasharray="3,3"/><polyline id="cumLine" class="cum-line" fill="none" stroke-width="2" points=""/><g id="rateBars"></g></svg></div>');P.push('<div class="legend"><span class="key cum">■ '+seriesCum+'</span><span class="key rate">■ '+seriesRate+" ("+sampleLbl+")</span></div>");P.push('<script nonce="'+nonce+'">');P.push(__ccsdRateChartJs());P.push("</script></body></html>");return P.join("");}function __ccsdRateChartCss(){return "body{background:var(--vscode-editor-background,#1e1e1e);color:var(--vscode-editor-foreground,#ddd);font:12px/1.4 -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;margin:0;padding:12px;}.hdr{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:10px;border-bottom:1px solid var(--vscode-editorWidget-border,#333);padding-bottom:6px;}.title{font-weight:600;font-size:14px;}.sid{color:var(--vscode-descriptionForeground,#888);font-family:ui-monospace,SFMono-Regular,monospace;}.stats{display:flex;gap:24px;margin-bottom:10px;}.stat .lbl{color:var(--vscode-descriptionForeground,#888);font-size:11px;text-transform:uppercase;letter-spacing:0.5px;}.stat .val{font-size:20px;font-weight:600;color:var(--vscode-editor-foreground,#fff);font-variant-numeric:tabular-nums;}.chart{background:var(--vscode-editorWidget-background,#181818);border:1px solid var(--vscode-editorWidget-border,#333);border-radius:4px;padding:8px;}svg{display:block;}.legend{margin-top:8px;display:flex;gap:20px;color:var(--vscode-descriptionForeground,#888);font-size:11px;}.key.cum{color:var(--vscode-chart-blue,#4fc3f7);}.key.rate{color:var(--vscode-chart-orange,#ffb74d);}.midline{stroke:var(--vscode-editorRuler-foreground,#444);}.cum-line{stroke:var(--vscode-chart-blue,#4fc3f7);}.rate-bar{fill:var(--vscode-chart-orange,#ffb74d);}";}function __ccsdRateChartJs(){return "var totalEl=document.getElementById('totalVal');var rateEl=document.getElementById('rateVal');var cumLine=document.getElementById('cumLine');var rateBars=document.getElementById('rateBars');var SVG_W=800,SVG_H=260,MID_Y=130,BAR_ZONE_H=110;function fmtT(n){n=n||0;if(n<1000)return String(Math.round(n));if(n<1e6)return(n/1e3).toFixed(1)+'k';if(n<1e9)return(n/1e6).toFixed(2)+'M';if(n<1e12)return(n/1e9).toFixed(2)+'B';return(n/1e12).toFixed(2)+'T';}function render(msg){if(!msg||msg.type!=='tick')return;var samples=msg.samples||[];if(samples.length===0){totalEl.textContent='\\u2014';rateEl.textContent='\\u2014';return;}totalEl.textContent=fmtT(msg.totalNow||0);rateEl.textContent=fmtT(msg.rateNow||0)+'/s';var n=samples.length;if(n<2){cumLine.setAttribute('points','');rateBars.innerHTML='';return;}var tMin=samples[0].t,tMax=samples[n-1].t||tMin+1;var tSpan=Math.max(1,tMax-tMin);var vMin=samples[0].total,vMax=samples[n-1].total;var vSpan=Math.max(1,vMax-vMin);var cumPts=[];for(var i=0;i<n;i++){var x=Math.round((samples[i].t-tMin)/tSpan*(SVG_W-20))+10;var y=Math.round(240-(samples[i].total-vMin)/vSpan*110);cumPts.push(x+','+y);}cumLine.setAttribute('points',cumPts.join(' '));var peak=Math.max(1,msg.peak||1);var barsHtml='';var bw=Math.max(2,Math.floor((SVG_W-20)/n));for(var j=0;j<n;j++){var x2=Math.round((samples[j].t-tMin)/tSpan*(SVG_W-20))+10;var h=Math.round(samples[j].d/peak*BAR_ZONE_H);var y2=MID_Y+10-h;var x2s=String(x2),y2s=String(y2),ws=String(bw-1),hs=String(Math.max(1,h));barsHtml+='<rect class=\\'rate-bar\\' x=\\''+x2s+'\\' y=\\''+y2s+'\\' width=\\''+ws+'\\' height=\\''+hs+'\\' fill-opacity=\\'0.75\\'/>';}rateBars.innerHTML=barsHtml;}window.addEventListener('message',function(e){render(e.data);});try{if(typeof acquireVsCodeApi==='function'){acquireVsCodeApi().postMessage({type:'ready'});}}catch(_){}";}`,
