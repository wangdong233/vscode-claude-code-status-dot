...
- 📊 **commandCenter oben 4-Lichter-Streifen** — oben mittig in der Titelleiste werden `🟢fertig 🟡läuft 🔵wartet 🔴unterbrochen` nebeneinander angezeigt (v0.1.13 ersetzt die v0.1.12 SBI rechts unten), jede Lampe begrenzt auf 0/1/2/3/N (>=4 zeigt N), bei 0 dunkel als ⚪. 🔵 = wartet auf Benutzereingabe (Permission/Question/Elicit, gespeist vom neuen Notification-Hook-Case). Fertig vor >5 Min zählt als idle (nicht grün). 20 Commands + 20 Menu-Items in CCs package.json injiziert; setContext schaltet die Sichtbarkeit alle 500ms um.
