...
- 📊 **4 blocos SBI coloridos embaixo (dígito dentro do bloco)** — o lado esquerdo da barra de status (`StatusBarAlignment.Left` + 4 blocos prioridade `-9996..-9999`, perto do centro visível) renderiza 4 **blocos coloridos cada um com o dígito dentro** lado a lado (v0.1.15 substitui o formato «emoji + dígito separado» da v0.1.14): 🟢concluído (bloco verde) / 🟡correndo (bloco amarelo) / 🔵pendente (bloco azul) / 🔴interrompido (bloco vermelho). Cada bloco limitado a 0/1/2/3/N (>=4 mostra N); quantidade>0 → bloco aceso (`statusBarItem.*Background` cor do tema + dígito branco dentro), quantidade=0 → bloco tênue (fundo transparente + «0» cinza, bloco continua visível). 🔵 = aguardando entrada do usuário (permissão/pergunta/elicit, alimentado pelo caso de hook Notification). Concluído há >5 min conta como idle (não verde). v0.1.15 usa **4 instâncias independentes runtime StatusBarItem + 4 cores integradas do tema** (sem parch no package.json do CC, o IIFE muta text/color/backgroundColor de cada bloco diretamente a cada 500ms) — as cores seguem o tema do VSCode, estáveis multiplataforma (sem dependência de fonte emoji).
