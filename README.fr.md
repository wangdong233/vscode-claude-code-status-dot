...
- 🔧 **Installation en une ligne** — `npx vscode-claude-code-status-dot` patche automatiquement l'extension CC, câble 9 hooks, copie les fichiers d'exécution ; idempotent, relançable à volonté
...
- 📊 **4 boules SBI emoji en bas (boule+chiffre, position fixe)** — le côté gauche de la barre d'état (`StatusBarAlignment.Left` + 4 slots priorité `-9996..-9999`, près du centre visible) affiche 4 **boules emoji avec un chiffre à côté de chacune** côte à côte (v0.1.16 restaure le style boule emoji de v0.1.14, en conservant la structure 4-SBI de v0.1.15 pour la stabilité de position) : 🟢terminé / 🟡en cours / 🔵en attente / 🔴interrompu. Le texte de chaque slot est `<boule><chiffre>` (la boule porte sa propre couleur — **le bloc coloré thématique de v0.1.15 + chiffre blanc est SUPPRIMÉ**) ; quantité plafonnée à 0/1/2/3/N (>=4 affiche N) ; quantité>0 → boule allumée (🟢/🟡/🔵/🔴 précolorées + chiffre juste à côté), quantité=0 → boule grise ⚪ + « 0 » (espace réservé correspondant à la largeur non-zéro, **position jamais décalée** — c'est l'avantage central du 4-SBI sur le SBI unique de v0.1.14 : le `🟢N 🟡N 🔵N 🔴N` de v0.1.14 faisait que toute la rangée se décalait à gauche/droite quand les largeurs de chiffres changeaient ; 4-SBI donne à chaque lumière son propre slot fixe `<boule><1-chiffre>` qui ne bouge jamais). 🔵 = en attente d'entrée utilisateur (permission/question/elicit, alimenté par le cas de hook Notification). Terminé depuis >5 min compte comme idle (pas vert). v0.1.16 utilise **4 instances indépendantes runtime StatusBarItem + boules emoji** (pas de patch du package.json CC, pas de bloc ThemeColor, l'IIFE mute le text de chaque slot directement toutes les 500ms) — la boule emoji porte sa propre couleur donc le chemin de rendu est plus simple que v0.1.15 (pas de backgroundColor, pas de cache de color, pas de bascule lit/dim).
...
5. Écrire les **9 événements hook** dans `~/.claude/settings.json` (marqués `# cc-status-dot-managed`, idempotents) ;
