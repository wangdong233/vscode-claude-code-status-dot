...
/** Byte range of a top-level key's value in a JSONC object literal. */
 ...
 */
export function findTopLevelKey(raw: string, key: string): KeyRange | null {
    let i = 0;
    let inString = false;
    let quote = "";
    let depth = 0;
 ...
 */
export function surgicalSetTopLevelKey(raw: string, key: string, valueJson: string): string {
 ...
 */
export function surgicalRemoveTopLevelKey(raw: string, key: string): string {
    const range = findTopLevelKey(raw, key);
    if (!range) return raw;
