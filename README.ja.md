...
**上部タブバーと左の「開いているエディター（Open Editors）」に各セッションの状態ドットを表示**<br>（🟡 実行中 / 🟢 完了 / 🔵 入力待ち / 🔴 中断 / ⚪ アイドル）
...
CC でプロンプトをひとつ送る:
- タブアイコンが 🟡 **黄** に → 完了で 🟢 **緑** に変わる。CC が権限承認 / question / elicit を要求するときはタブが 🔵 **青** に（CC ネイティブの青ドットを表示、**上書きしない**）
- 下部ステータスバーの 🟡 実行中ライトが +1、完了で 🟢 完了ライトが +1、入力待ちで 🔵 入力待ちライトが +1
...
各 CC セッションのタブアイコンが状態に応じて変色、**上部タブバーと左上の「開いているエディター」ビューの両方に表示**。running / done / idle は静的ドット、interrupted は赤の速ブリンクで警告、CC が権限承認ダイアログを表示するときは reader がアイコンを譲って CC ネイティブの青ドットを表示（**上書きしない**）。結果としてタブ上には 5 色（⚪ 灰 / 🟡 黄 / 🟢 緑 / 🔴 赤 / 🔵 青）が並ぶ —— CC ネイティブ（青 / オレンジの 2 点のみ）より完全：idle / running / done / interrupted に加え、permission（青、CC ネイティブに譲渡）まで可視化。
...
> ⚪ **アイドル（灰）は下部集計に含まれない** —— idle = アクティブなセッションなしを意味するため、集計欄に出す意味がない（タブ上では灰ドットで表示される）。
...
- [`docs/STATES.md`](docs/STATES.md) —— **状態契約（唯一の真実源）**: 5 状態（灰 / 黄 / 緑 / 赤 / 青） / イベントマッピング / IPC / 通知
