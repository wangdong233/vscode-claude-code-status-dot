...
- 📊 **SBI 4 farbige Blöcke unten (Zahl im Block)** — die linke Seite der Statusleiste (`StatusBarAlignment.Left` + 4 Blöcke Priorität `-9996..-9999`, nahe der sichtbaren Mitte) zeigt 4 **farbige Blöcke mit der Ziffer jeweils innen** nebeneinander (v0.1.15 ersetzt v0.1.14s „Emoji-Ball + Ziffer getrennt"): 🟢fertig (grüner Block) / 🟡läuft (gelber Block) / 🔵wartet (blauer Block) / 🔴unterbrochen (roter Block). Jeder Block begrenzt auf 0/1/2/3/N (>=4 zeigt N); Anzahl>0 → Block leuchtet (`statusBarItem.*Background` Theme-Farbe + weiße Ziffer innen), Anzahl=0 → Block dunkel (transparenter Grund + graue „0", Block bleibt sichtbar). 🔵 = wartet auf Benutzereingabe (Permission/Question/Elicit, gespeist vom Notification-Hook-Case). Fertig vor >5 Min zählt als idle (nicht grün). v0.1.15 verwendet **4 unabhängige runtime StatusBarItem-Instanzen + 4 eingebaute Theme-Farben** (kein CC-package.json-Patch, die IIFE mutiert pro Block text/color/backgroundColor direkt alle 500ms) — Farben folgen dem VSCode-Theme, plattformübergreifend stabil (keine Emoji-Font-Abhängigkeit mehr).
