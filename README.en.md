...
- 📊 **Bottom status bar 4-light aggregate** — the window's bottom status bar (left half, near the center) renders **one single StatusBarItem** holding four emoji lights separated by small spaces: 🟢done · 🟡running · 🔵pending · 🔴interrupted. Each light is a `<ball><count>` pair; counts cap at `0/1/2/3/N` (`N` for 4+). When a count is `0`, both the ball (gray ⚪) and the digit go dim; when non-zero, the ball takes its color (🟢/🟡/🔵/🔴) and the digit lights up. 🔵 = awaiting user input (permission / question / elicit, fed by the Notification hook). The four slots are fixed in position — counts changing never shifts the layout (VSCode's status bar applies `tabular-nums`, so digits 0-9 are equal-width regardless of font).
...
7. **New in v0.2.0**: detects every VS Code-family CLI on PATH (`code`, `code-insiders`, `cursor`, `codium`) and runs `code --install-extension` for the **companion .vsix** (`cc-status-dot-companion`) into each; also copies `patch.js` to `INSTALL_DIR/patch.js` so the companion can silently re-patch after a CC auto-update.
...
- **CC auto-update overwrite**: patched `extension.js` overwritten by the original → **since v0.2.0 the companion extension auto-re-runs the patcher + prompts a reload** (see FAQ); without the companion, re-run the command manually to restore.
