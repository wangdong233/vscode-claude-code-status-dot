...
**⚙ Zahnrad-Abzeichen für Hintergrund-Aufgaben**　Direkt unter dem Statuspunkt erscheint ein kleines graues Zahnrad = in dieser Session laufen noch Shell-Hintergrund-Aufgaben (z. B. ein mit `nohup` gestarteter Dev-Server oder Test-Watcher — die Art, die die Farbzustände nicht ausdrücken). Das Zahnrad verschwindet, sobald alle Aufgaben enden; die Fünf-Zustands-Farbsemantik bleibt unberührt.
