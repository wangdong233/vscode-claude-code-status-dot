...
- 📊 **4 bolas SBI emoji embaixo (v0.1.17: SBI único, concatenação compacta, espaçamento 0px, dígitos nunca se deslocam)** — o lado esquerdo da barra de status (`StatusBarAlignment.Left` + um único SBI com prioridade `-9996`, perto do centro visível) renderiza **uma linha compacta de 4 bolas emoji** (v0.1.17 colapsa a linha 4-SBI do v0.1.16 de volta a um único SBI cujo texto é a concatenação de 4 segmentos `🟢3🟡1⚪0⚪0` — causa raiz: o `statusbarpart.css` do VSCode endurece `margin:0 3px;padding:0 5px` por SBI (~6-16px de espaçamento entre SBIs adjacentes, não controlável via API pública; o «espaçamento frouxo» que você via no v0.1.16 é um limite hard do framework, só SBI único dá compacidade real): 🟢concluído / 🟡correndo / 🔵pendente / 🔴interrompido. O token de cada luz é `<bola><dígito>` (a bola carrega sua própria cor); os 4 tokens são concatenados **sem separador** (**espaçamento de 0px entre as luzes** — a largura da linha encolhe de ~120px para ~70px); quantidades limitadas a 0/1/2/3/N (>=4 mostra N); quantidade>0 → bola acesa (🟢/🟡/🔵/🔴 pré-coloridas + dígito logo ao lado), quantidade=0 → bola cinza ⚪ + «0». 🔵 = aguardando entrada do usuário (permissão/pergunta/elicit, alimentado pelo caso de hook Notification). Concluído há >5 min conta como idle (não verde). **«Os dígitos nunca se deslocam» é garantido pela regra CSS `font-variant-numeric:tabular-nums` do próprio VSCode** (statusbarpart.css aplica tabular-nums a cada item da barra de status — dígitos ASCII 0-9 têm a mesma largura em qualquer fonte), independente da renderização da fonte emoji. v0.1.17 usa **1 StatusBarItem em runtime + texto concatenado** (sem parch no package.json do CC, sem bloco ThemeColor, o IIFE muta o text diretamente a cada 500ms).
