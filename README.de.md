...
> **Zuverlässigkeits-Garantie**: Wenn CCs Auto-Update den Patch überschreibt, repariert die Companion-Erweiterung ihn automatisch neu + schlägt `Reload Window` vor (unbemerkt); vor dem Patchen wird die komplette 2,6 MB `extension.js` mit `node --check` validiert + atomares Schreiben (**CC wird nie beschädigt**); `--revert` stellt mit einem Klick ohne Nebeneffekte wieder her; die Laufzeitkopie liegt unter `~/.claude/cc-status-dot/` (Quelle löschen / Cache leeren / CC-Update beeinträchtigen das nicht); während Workflow-Subagent-Läufen bleibt die Haupt-Session 🟡 (kein falsches Grün).
