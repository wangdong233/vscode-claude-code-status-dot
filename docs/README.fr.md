...
**⚙ Badge engrenage pour tâches en arrière-plan**　Un petit engrenage gris apparaît juste sous le point d'état = il reste dans cette session des tâches en arrière-plan de type shell en cours (p. ex. un dev server ou test watcher lancé avec `nohup` — celles que les états couleur ne peuvent pas exprimer). L'engrenage disparaît dès que toutes les tâches s'arrêtent ; la sémantique des cinq états couleur reste intacte.
...
- **Fragilité des ancres minified** : le patch localise le code CC via une ancre à deux niveaux (littéral exact en chemin rapide + regex tolérante en repli — ancrée sur les chaînes du protocole IPC, immunisée contre les renommages du minifieur) ; seul un changement structurel déclenche « Anchor mismatch » et un refus d'écriture sans empreinte ; le `extension.js` complet de ~3 Mo est validé par `node --check` avant écriture (garde `assertCompiles`, un IIFE cassé est refusé), écriture atomique (`.tmp` + rename), `INJECT_VERSION` réinjecté automatiquement — **CC ne sera jamais brické**.
