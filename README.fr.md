...
- 📊 **Bandeau 4 lumières en haut du commandCenter** — en haut au centre de la barre de titre s'affichent `🟢terminé 🟡en cours 🔵en attente 🔴interrompu` côte à côte (v0.1.13 remplace le SBI bas-droite de v0.1.12), chaque lumière plafonnée à 0/1/2/3/N (>=4 affiche N), tamisée ⚪ à 0. 🔵 = en attente d'entrée utilisateur (permission/question/elicit, alimenté par le nouveau cas de hook Notification). Terminé depuis >5 min compte comme idle (pas vert). 20 commandes + 20 items de menu injectés dans le package.json de CC ; setContext bascule la visibilité toutes les 500ms.
