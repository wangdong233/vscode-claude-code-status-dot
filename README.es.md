...
- 🔧 **Instalación en una línea** — `npx vscode-claude-code-status-dot` aplica el parche a la extensión de CC, conecta 9 hooks, copia los archivos de runtime; idempotente, se puede ejecutar varias veces
...
- 📊 **4 bolas SBI emoji abajo (bola+dígito, posición fija)** — el lado izquierdo de la barra de estado (`StatusBarAlignment.Left` + 4 slots prioridad `-9996..-9999`, cerca del centro visible) renderiza 4 **bolas emoji con un dígito al lado de cada una** uno al lado del otro (v0.1.16 restaura el estilo de bola emoji de v0.1.14, conservando la estructura 4-SBI de v0.1.15 para posición estable): 🟢hecho / 🟡corriendo / 🔵pendiente / 🔴interrumpido. El texto de cada slot es `<bola><dígito>` (la bola lleva su propio color — **se ELIMINA el bloque con color del tema de v0.1.15 + dígito blanco**); cantidades limitadas a 0/1/2/3/N (>=4 muestra N); cantidad>0 → bola iluminada (🟢/🟡/🔵/🔴 precoloreadas + dígito justo al lado), cantidad=0 → bola gris ⚪ + «0» (marcador de posición coincide con ancho no-cero, **la posición nunca cambia** — esta es la ventaja central de 4 SBI sobre el SBI único de v0.1.14: el `🟢N 🟡N 🔵N 🔴N` de v0.1.14 hacía que toda la fila se desplazara al cambiar los anchos de los dígitos; 4 SBI da a cada luz su propio slot fijo `<bola><1-dígito>` que nunca se mueve). 🔵 = esperando entrada del usuario (permiso/pregunta/elicit, alimentado por el caso de hook Notification). Hecho de >5 min cuenta como idle (no verde). v0.1.16 usa **4 instancias independientes runtime StatusBarItem + bolas emoji** (sin parchear el package.json de CC, sin bloque ThemeColor, el IIFE muta el text de cada slot directamente cada 500ms) — la bola emoji lleva su propio color por lo que la ruta de render es más simple que v0.1.15 (sin backgroundColor, sin caché de color, sin cambio lit/dim).
...
5. Escribir **9 eventos de hook** en `~/.claude/settings.json` (con la marca `# cc-status-dot-managed`, idempotente);
