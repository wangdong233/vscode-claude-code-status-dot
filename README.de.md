...
- 📊 **SBI 4-Lichter-Streifen unten** — die linke Seite der Statusleiste (`StatusBarAlignment.Left` + sehr negative Priorität `-9999`, nahe der sichtbaren Mitte) zeigt `🟢fertig 🟡läuft 🔵wartet 🔴unterbrochen` nebeneinander (v0.1.14 ersetzt die v0.1.13 commandCenter-Lösung oben, die nach Reload/Restart unzuverlässig war), jede Lampe begrenzt auf 0/1/2/3/N (>=4 zeigt N), bei 0 dunkel als ⚪. 🔵 = wartet auf Benutzereingabe (Permission/Question/Elicit, gespeist vom Notification-Hook-Case). Fertig vor >5 Min zählt als idle (nicht grün). v0.1.14 vereinfacht zu **einem einzigen runtime StatusBarItem** — kein CC-package.json-Patch, die IIFE mutiert den Text direkt alle 500ms; die setContext→when-Kette aus v0.1.13 ist entfernt.
