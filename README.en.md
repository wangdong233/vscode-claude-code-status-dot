...
- 📊 **Bottom SBI 4 emoji balls (v0.1.17: single SBI, compact concat, 0px gap, digits never shift)** — the status bar's left side (`StatusBarAlignment.Left` + a single SBI at priority `-9996`, near the visible center) renders **one tight row of 4 emoji balls** (v0.1.17 collapses the v0.1.16 4-SBI row back into a single SBI whose text is the 4-segment concat `🟢3🟡1⚪0⚪0` — root cause: VSCode's `statusbarpart.css` hardcodes `margin:0 3px;padding:0 5px` per SBI (~6-16px gap between adjacent SBIs, uncontrollable via public API; the 4-SBI "loose gap" you saw under v0.1.16 is a framework-level hard limit, only single-SBI gives true compactness): 🟢done / 🟡running / 🔵pending / 🔴interrupted. Each light's token is `<ball><digit>` (ball carries its own color); the 4 tokens are concatenated with no separator (**0px inter-light gap** — row width shrinks from ~120px to ~70px); counts cap at 0/1/2/3/N (>=4 shows N); count>0 → ball lit (🟢/🟡/🔵/🔴 pre-colored + digit right beside it), count=0 → gray ball ⚪ + "0". 🔵 = awaiting user input (permission/question/elicit, fed by the Notification hook case). Done older than 5 min counts as idle (not green). **"Digits never shift" is guaranteed by VSCode's own CSS `font-variant-numeric:tabular-nums`** (statusbarpart.css applies tabular-nums to every statusbar item — ASCII digits 0-9 are equal-width in any font) — independent of emoji font rendering. v0.1.17 uses **1 runtime StatusBarItem + concatenated text** (no CC package.json patch, no ThemeColor block, the IIFE mutates the SBI's text directly every 500ms).
...
- **SBI priority namespace not owned** (v0.1.17, **collision window shrinks from 4 units to 1**): the bottom SBI occupies the single priority `-9996` (a point, down from v0.1.16's `-9996..-9999` 4-unit range). The VSCode StatusBarItem API has no extension-level namespace/ownership — another extension declaring the same priority could push our SBI to a corner. **v0.1.17's single-SBI architecture eliminates v0.1.16's "row split by an outsider" failure mode** (v0.1.16's 4 independent SBIs could be split when another extension's SBI wedged between done and interrupted; v0.1.17's whole row is one SBI, so an external insertion can only land on the side, never between the 4 lights); the collision probability also drops to 1/4. Rare in practice; documented honestly in STATES.md §7.5.
- **Emoji font-stack dependency** (v0.1.17 inherited from v0.1.16): the bottom SBI dots are emoji glyphs (🟢🟡🔵🔴⚪) that depend on the system emoji font stack — macOS (Apple Color Emoji) / Windows 10+ (Segoe UI Emoji) / mainstream Linux (Noto Color Emoji) render them in color; Win7 / some headless Linux / emoji-font-less remote SSH environments may render them as monochrome glyphs or tofu boxes. The v0.1.15 `ThemeColor` path was cross-platform-stable; v0.1.16+'s return to emoji balls is a deliberate tradeoff (user aesthetic preference > cross-platform uniformity).
