...
**② サイドバー CC お気に入り / CC アーカイブビュー**　Explorer サイドに CC Favorites + CC Archive の 2 つのビューを新設（相互排他：1 つのセッションはどちらか一方にのみ存在）；お気に入りビューはよく使うファイル / セッションを一緒に pin、アーカイブビューは当面使わないセッションを収納。セッションアイコンは open=塗りつぶしバブル / closed=アウトラインバブル、クリックで該当 panel にジャンプまたは新 panel に resume；行内ボタンで相互切替 —— お気に入りビュー [アーカイブ][開く][削除]、アーカイブビュー [お気に入り][開く][削除]、アーカイブ / お気に入りをクリックすると自動的に反対側のビューへ移動。閉じたセッションを右クリックで `claude -r <sid>` コマンドをコピー可能。
