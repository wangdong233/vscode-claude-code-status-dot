...
**⚙ Background-task gear badge**　A small grey gear appears directly below the status dot = this session still has shell-type background tasks in flight (e.g. a dev server or test watcher started with `nohup` — the kind the color states can't express). The gear disappears once all tasks stop; the five-state color semantics are untouched.
