...
**⚙ Insígnia de engrenagem para tarefas em segundo plano**　Uma pequena engrenagem cinza aparece logo abaixo do ponto de estado = esta sessão ainda tem tarefas em segundo plano do tipo shell em execução (por exemplo, um dev server ou test watcher iniciado com `nohup` — o tipo que os estados de cor não conseguem expressar). A engrenagem desaparece quando todas as tarefas param; a semântica das cinco estados de cor permanece intacta.

**② Vista CC Favorites / CC Archive na barra lateral**　O Explorer ganha duas novas vistas CC Favorites + CC Archive (mutuamente exclusivas: uma sessão só pode estar em uma); a vista de favoritos fixa arquivos/sessões usados com frequência num só lugar, a vista de arquivo guarda as sessões que você não está usando no momento. O ícone da sessão mostra open=balão de chat sólido / closed=balão só com contorno; clique salta para a sessão ou faz resume abrindo num novo painel; botões inline alternam entre as vistas — vista de favoritos [Arquivar][Abrir][Remover], vista de arquivo [Favoritar][Abrir][Remover], clicar em Arquivar/Favoritar remove automaticamente da outra vista. Clique direito numa sessão fechada permite copiar o comando `claude -r <sid>`. As linhas das duas visões podem ser **reordenadas arrastando** — soltar sobre uma linha insere antes dela, soltar após a última move para o fim. Após o primeiro arrasto, a lista passa a ordem manual (novas entradas continuam aparecendo no topo); soltamentos sem mudança não gravam em disco.
...
> Em execução é amarelo estático (sem animação); interrompido mantém o pisca vermelho rápido de alerta. O contrato completo de estados (eventos / SVG / IPC / notificações) está em [`docs/STATES.md`](STATES.md).
...
- **Fragilidade da âncora minificada**: o patch localiza o código do CC com uma âncora em dois níveis (literal exato como via rápida + regex tolerante como fallback — ancorada nas strings do protocolo IPC, imune a renomeações do minificador); apenas uma mudança estrutural devolve "Anchor mismatch" e se recusa a escrever sem deixar vestígios; antes de escrever o extension.js ainda roda `node --check` no arquivo completo de ~3MB (guardião assertCompiles, IIFE inválido é recusado antes de escrever), escrita atômica (`.tmp` + rename), `INJECT_VERSION` reinjeta automaticamente — **nunca corrompe o CC**.
...
- [`docs/STATES.md`](STATES.md) — **Contrato de estados (fonte única de verdade)**: cinco estados (cinza/amarelo/verde/vermelho/azul) + bloco agregado de 4 luzes na barra inferior / mapeamento de eventos / IPC / notificações
- [`docs/DESIGN-injection.md`](DESIGN-injection.md) — Princípio da injeção do ícone (âncora / IIFE / associação de SVG)
- [`docs/USAGE.md`](USAGE.md) — Guia de uso (instalação / troubleshooting / restauração)
