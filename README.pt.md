...
- 🔧 **Instala com uma linha** — `npx vscode-claude-code-status-dot` faz o patch automático da extensão CC, conecta 9 hooks, copia os arquivos de runtime; idempotente, pode rodar de novo
...
- 📊 **4 luzes agregadas na barra de status inferior** — a barra de status inferior da janela (lado esquerdo, perto do centro visível, `StatusBarAlignment.Left` com um único `StatusBarItem` de prioridade `-9996`) renderiza **um bloco compacto de 4 luzes separadas por pequenos espaços**: 🟢concluído / 🟡correndo / 🔵pendente / 🔴interrompido. Cada luz é seguida de um dígito de contagem (limitado a 0/1/2/3/N, onde N≥4); quantidade=0 → bola cinza ⚪ + dígito apagado; quantidade>0 → bola colorida 🟢/🟡/🔵/🔴 + dígito aceso. As 4 posições são fixas — os dígitos mudam sem deslocar as luzes (garantido pelo `font-variant-numeric: tabular-nums` que o próprio VSCode aplica a cada item da barra de status; dígitos ASCII 0-9 têm a mesma largura em qualquer fonte, independentemente da renderização da fonte emoji). 🔵 = aguardando entrada do usuário (permissão/pergunta/elicit, alimentado pelo hook `Notification`, contador independente do estado). Concluído há >5 min conta como idle (não verde). O IIFE muta o texto diretamente a cada 500ms (1 `StatusBarItem` em runtime + texto concatenado, sem parch no `package.json` do CC, sem bloco `ThemeColor`).
...
### 📊 Quatro luzes agregadas na barra de status inferior

A barra de status inferior da janela (perto do centro, `StatusBarAlignment.Left`) mostra **um único bloco compacto com 4 luzes**, separadas por pequenos espaços: 🟢concluído · 🟡correndo · 🔵pendente · 🔴interrompido. Cada luz é seguida de um dígito (contagem limitada a 0/1/2/3/N, onde N≥4): quando a contagem é 0 a luz fica apagada (bola cinza ⚪ + dígito), quando >0 fica acesa (bola colorida + dígito). As posições das 4 luzes são fixas — os dígitos mudam sem deslocar as luzes, graças ao `font-variant-numeric: tabular-nums` nativo da barra de status do VSCode.

🔵 (pendente) é **contada de forma independente do estado**: representa sessões aguardando entrada do usuário (permissão/pergunta/elicit), alimentada pelo hook `Notification`. As demais cores seguem o estado de cada sessão, com autolimpeza em três níveis: `done` há mais de 5 min → idle (desliga o verde); `running` cujo `mtime` passa de 30 min → idle (sessão travada); `interrupted` há mais de 24 h → idle.
