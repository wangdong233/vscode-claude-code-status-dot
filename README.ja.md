...
**① タブ 5 状態ドット**　各 CC セッションタブの Claude アイコンが状態に応じて変色 —— 🟡 実行中 / 🟢 完了 / 🔴 中断（速ブリンク）/ ⚪ アイドル / 🔵 入力待ち（CC が権限承認を求めるときは CC ネイティブの青ドットに譲る、**上書きしない**）；お気に入り登録済みのセッションタブタイトルには **★** プレフィックス + アイコン下部に金線を付加。上部タブバー + 左側「開いているエディター」の両方に表示、両者は同期。
...
- **v0.5.36 新機能：タブ切り替えで即座に追従** —— 別のセッションに切り替えると、token SBI が即座に新セッションのデータを反映（`__ccsdSidToPanel` をスキャンしてリアルタイム活動 panel を検出 + イベント駆動リフレッシュ、お気に入り星印マークと同等の仕組み）；**初期化中**のセッション（sid 未捕捉）に切り替えると ⟳ loading を表示、旧セッションの数字は残留しない。ロード完了後、2 セッション間の相互切り替えは **loading の点滅なし**（即時切り替え）
...
### ⭐ CC Favorites ビュー（v0.4.0+）+ tab 右クリック / 金線マーク（v0.5.0+）

VSCode エクスプローラーサイドバーに **CC Favorites** ビューを追加 —— よく使うファイルと CC セッションを一緒に pin し、パネル跨ぎ / 再起動跨ぎで素早く戻れる。

- **ファイルを追加**: Explorer で任意のファイルを右クリック → **CC Favorites: Add/Remove File**（設定項目 `ccStatusDot.fav.includeInExplorerContextMenu` はデフォルトでオン、メニューが混雑する場合はオフ可）。
- **CC セッションを追加**（3 つの入り口）:
  - コマンドパレットで **CC Favorites: Star/Unstar Current CC Tab** を検索 —— 現在アクティブなセッションを Favorites に追加 / 削除。
  - コマンドパレットで **CC Favorites: Pick CC Session to Star/Unstar**（v0.5.9+）を検索 —— QuickPick が開いている全 CC セッションをリスト表示（お気に入り済みの ★ が先頭）、1 つ選ぶと toggle、**現在アクティブな tab に依存しない**、セッション内でお気に入りに登録する信頼できる入り口。
  - **ステータスバー ★ ボタン（v0.5.10+、最も手軽）** —— 右下ステータスバー（token カウント横）に ★/☆ ボタン、**ワンクリック**で現在アクティブな CC セッションをお気に入り追加 / 削除: お気に入り済みは塗りつぶし金星 ★（金色、金線と整合）、未お気に入りは中空 ☆ を表示。常に現在アクティブなセッションに作用（webview write-once / 右クリックが tab を誤認するプラットフォーム制限を回避）、クリックで即反転（tab 切り替え後 ≤500ms で状態に追従、v0.5.11）；アクティブな CC セッションがないときは自動的に非表示になる。
  - Explorer の **Open Editors** 領域で CC tab を右クリック → **CC お気に入りに追加 / 削除**（動的ラベル、設定項目 `ccStatusDot.fav.includeInExplorerContextMenu`）。
- **★ タイトルプレフィックス（v0.5.9+）**: お気に入り済みの CC セッションでは、tab **タイトル**の前に自動的に `★ ` を付加（5 状態ドットの色 / 形は変わらず、金線マークも維持）。IIFE が 500ms ごとに `favorites.json` から同期（mtime キャッシュ → お気に入り書き込み後 ≤1s 以内に表示）。v0.5.8 の「webview 内クリック可能星マーク」はフォレンジック検証でアーキテクチャ不可と判明（CC は panel 生成時に一度だけ webview.html を設定、再設定はページ全体再読み込みを引き起こしセッションを破壊）し廃止、タイトルプレフィックスはその reload-free 代替。
- **ジャンプ**: ファイルノードをクリック → 該当ファイルへジャンプ（行番号位置指定付き）；**セッションノードをクリック → 開いているものは切り替え、未開のものは復元して開く（新 panel に resume、v0.5.11+）**；閉じたセッションを右クリック → **Copy 'claude -r <sid>'** で resume コマンドをクリップボードへコピー（ターミナルフォールバック）。
- **ブラウズ**: コマンドパレット **CC Favorites: Browse** で QuickPick キーボードナビゲーション（お気に入り登録済みアイテムを開く）。
- **金線マーク（v0.5.0+）**: お気に入り済みの CC セッションでは、tab icon の下部に細い金線を追加（5 状態ドットの色 / 形は完全に変わらず）、IIFE が 500ms ごとに `favorites.json` から自動同期。
- **セッションツリーアイコンの識別（v0.5.36 新）**: サイドバー CC Favorites ビューで、**open セッション**（初期化済みで使用中）は塗りつぶし薄灰チャットバブル（前景塗りつぶし + 背景アウトライン）、**closed セッション**は単一アウトラインバブルを表示 —— どのセッションがまだ生きているか、どれが閉じたかをひと目で判別。

お気に入りは `~/.claude/cc-tab-status/favorites.json` に保存（atomic 書き込み、再起動跨ぎで保持）。完全な設計は [`docs/FAVORITES-DESIGN.md`](docs/FAVORITES-DESIGN.md) を参照。

> v0.5.11 以降、閉じたセッションのクリックは直接 panel に resume する —— CC 自身の `claude-vscode.editor.open(sid)` → `createPanel(sid)` を経由、CLI 起動時に `--session-id=<sid>` を付与してそのセッションの履歴を読み込む。右クリック Copy cmd はターミナルフォールバックとして保持。
...
1. **右下の token SBI をクリック** → QuickPick 設定パネルが立ち上がる（上記「🖼️ ひと目で分かる」の ⑤ 参照）——グラフィカルに統計ウィンドウ / 表示モード / 通知 / サウンドを切り替え、トークン数コピー / 統計リセット / 状態ディレクトリを開く / 設定を開くことも可能。変更は自動的に `settings.json` に書き込まれ、パネルは VSCode の UI 言語に追従（中 / 英 / 日 / 独 / 西 / 仏 / 葡 / 露、未知の言語は en にフォールバック）。
