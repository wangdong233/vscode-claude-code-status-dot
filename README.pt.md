...
- 📊 **Faixa de 4 luzes no topo do commandCenter** — no centro superior da barra de título renderiza `🟢concluído 🟡correndo 🔵pendente 🔴interrompido` lado a lado (v0.1.13 substitui o SBI inferior direito da v0.1.12), cada luz limitada a 0/1/2/3/N (>=4 mostra N), tênue ⚪ em 0. 🔵 = aguardando entrada do usuário (permissão/pergunta/elicit, alimentado pelo novo caso de hook Notification). Concluído há >5 min conta como idle (não verde). 20 comandos + 20 itens de menu injetados no package.json do CC; setContext alterna a visibilidade a cada 500ms.
