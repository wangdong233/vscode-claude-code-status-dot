 ...
 *  emits these bytes inside itself), so strip is a closed-interval removal
 *  with zero ambiguity. The legacy banner line above stays byte-identical
 *  so v0.5.x companions (marker grep + :version:hash parse at :501) keep
 ...
 *  (v0.5.13 conservative guard: "cannot prove stale"). mtime witnesses EVERY
 *  hook fire, so mtime staler than 2h with no last_ts = dead spawn. 2h > 30m
 *  (mtime is a strictly broader witness than last_ts's assistant rows) and
 ...
 *  blips leave EH timers running (no gap); App-Nap/forward-clock stalls
...
// at a throwaway CC copy instead of the live install, exclusively.
...
function buildIIFE(resDir: string): string {
    ...
    const bodyLines = [
        ...
        // steady state (99.6% reduction). See git history (pre-0.5.10
        // CHANGELOG, v0.2.9) + docs/STATES.md §9 perf section.
        ...
        // independent caches, two independent (mt,sz) change signals.
        `var ARCF=pth.join(DIR,"archive.json");`,
        ...
        ,
        /*v0.5.52 sleep ledger: MarkTick is the FIRST statement of BOTH 500ms tick
        bodies (§F __ccsdSbiTick + §H per-panel timer) — after a system sleep the
        first post-wake tick fires LATE, and the gap (minus a small epsilon baked
        into the >GAP test) is recorded as a suspension interval. AgeAdj builds a per-witness closure (R1 clamp: only overlap with [ts,now] counts). R1: the ledger is also deleted on last-panel-out (§Z) so a panel-less dwell records nothing.
        subtracts the recorded overlap from a wall-clock age so decay predicates
        measure AWAKE time. EH frozen during real sleep cannot fire timers → no
        false negatives; ledger is per-EH-lifetime (restart loses history → ages
        fall back to wall clock, same as pre-v0.5.52 — bounded, documented).*/
        `function __ccsdMarkTick(){try{var L=globalThis.__ccsdSuspLedger;if(!L){L=globalThis.__ccsdSuspLedger={t:Date.now(),iv:[]}}var n=Date.now();if(n>L.t&&(n-L.t)>CCSD_SUSPEND_GAP_MS){L.iv.push([L.t,n]);if(L.iv.length>CCSD_LEDGER_MAX_IV)L.iv.shift()}L.t=n}catch(_){}}`,
        `function __ccsdAgeAdj(n){/*v0.5.52 R1 per-witness clamp: returns a function ts→effective-now FOR THAT ts (subtracts only suspension overlapping [ts,n] — sleep PREDATING a witness must not shrink its age: a session spawned after a wake must not inherit the night as credit).*/try{var L=globalThis.__ccsdSuspLedger;if(!L)return function(ts){return n};var iv=L.iv;return function(ts){var s=0;for(var i=0;i<iv.length;i++){var a=iv[i];if(a[1]<=ts)continue;var lo=a[0]>ts?a[0]:ts;s+=a[1]-lo}return n-s}}catch(_){return function(ts){return n}}}`,
        ...
        `__ccsdMarkTick();var __adj=__ccsdAgeAdj(Date.now());`,
        `try{`,
        ...
        /*§F four-light decay (done>5min / interrupted>7d / running-stale) — unified predicate __ccsdDecayState (decayInterrupted=true — four-light aggregates interrupted at 7d per STATES.md §7.5); see its declaration for full rationale.*/ `st=__ccsdDecayState(st,since,j,Date.now(),true,__mt,__adj);`,
        ...
        // v0.6 seam: isPend now ORs the two NEW observer-sourced pending terms
        // (design §6.4 five-state table) alongside the three v0.5.x sources —
        // __ccsdToolPermSet (outbound tool_permission_request, finer-grained
        // than rename_tab's hasPendingPermissions) and __ccsdWireState
        // (update_session_state.state==="waiting_input" — a wire state the
        // hook files never see). Both sets are created by the seam prelude and
        // are ABSENT in any anchor-era install, so the && guards keep this
        // body backward-compatible with a legacy IIFE. st!=="idle" gate
        // unchanged (a decayed session never false-sticks blue).
        `var isPend=((j.pending===true)||(__ps&&__ps[files[i].slice(0,-5)]===true)||(globalThis.__ccsdUserDialogSet&&globalThis.__ccsdUserDialogSet[files[i].slice(0,-5)]===true)||(globalThis.__ccsdToolPermSet&&globalThis.__ccsdToolPermSet[files[i].slice(0,-5)]===true)||(globalThis.__ccsdWireState&&globalThis.__ccsdWireState[files[i].slice(0,-5)]==="waiting_input"))&&st!=="idle";`,
...
// flow — a hot zone CC actively rewrites (3 structural breaks in 47 days,
// MTBF ~10-12 days and worsening on all three axes; see the design doc §3.1).
// The seam instead PREPENDS a self-contained prelude at bundle head (zero
// anchors — always succeeds) that:
//   L1 rebinds the module-local `require` parameter so require("vscode")
//      returns a wrapped namespace (per-module, no Module._load, no
//      cross-extension leakage; mirrors .resolve/.main/.cache),
//   L2 wraps window.createWebviewPanel / registerWebviewViewProvider /
//      registerWebviewPanelSerializer (dual-name deserializeWebviewPanel /
//      resolveWebviewPanel — D1's single-name was wrong, D2-verified against
//      vscode.d.ts 1.136 + CC 2.1.259) to capture the REAL surface objects,
//   L3 observes protocol messages both ways (inbound onDidReceiveMessage
//      pure observer; outbound postMessage instance shadow, whitelist-gated),
//   L4 reduces them onto the same globalThis bridges the companion consumes
//      (binding layer = panel family ONLY — HIGH#1; sidebar/sessions views
//      observe, never bind, never decorate),
//   L5 runs the ported §A..§Z machinery (buildIIFEInner) against a synthetic
//      per-panel `t` context — the anchor era read sid/title/panelTab from
//      the handler's `this`; the seam supplies the identical shape from
//      observed messages, so the battle-tested tick/SBI/decay logic is
//      byte-reused, not rewritten,
//   L7 writes a layered heartbeat (seam-state-<pid>.json) whose obs counters
//      are an end-to-end canary for every escape the static gates cannot see.
// Stability contract (design §3.2): everything the seam keys on is either a
// vscode public API name (S2, contract-guaranteed) or a protocol string that
// crosses the webview IPC boundary (S1, unminifiable). Nothing keys on CC's
// internal control flow ever again.
// ---------------------------------------------------------------------------
/** buildIIFE's body WITHOUT the legacy `(function(t){…})(this)` wrapper and
 *  WITHOUT the banner — the exact byte range between the two compile-time
 *  wrapper literals. The seam prelude embeds this as the body of
 *  `function __ccsdPanelInit(t){…}` (t passed as a real parameter; the
 *  synthetic context supplies panelTab/__ccsdSid/__ccsdTitle/context — the
...
function buildIIFEInner(resDir: string): string {
    ...
    if (i <= 0 || j <= i || j + tail.length !== stamped.length) {
        throw new Error("buildIIFE wrapper shape drifted — buildIIFEInner slicing contract broken");
    }
...
function buildSeamPrelude(): string {
    ...
    const idirLiteral = JSON.stringify(INSTALL_DIR);
    const verLiteral = JSON.stringify(INJECT_VERSION);
    const lines = [
        // === L0 prelude open + module-scope capture ===
        // RAW: the module wrapper's ORIGINAL require binding, captured before
        // the rebind below. Every seam-internal need (fs for the heartbeat)
        // goes through it so seam health is independent of rebind success.
        `(function(){`,
        `var G=globalThis;`,
        `var RAW=require;`,
        `var IDIR=${idirLiteral};`,
        `var RESDIR=${resLiteral};`,
        `var SEAMV=${verLiteral};`,
        // === L7 heartbeat (defined first — every degrade path below feeds it) ===
        // Layered per design §7.2: prelude-alive (v/pid) -> surface-alive
        // (surfaces/armed) -> channel-alive (obs per-message-type counters =
        ...
        `var HB={v:SEAMV,pid:process.pid,bootTs:Date.now(),writtenTs:0,surfaces:{panel:0,sidebar:false,sessionsList:false},armed:0,envelopeFail:0,obs:{update_session_state:0,seeded:0,rename_tab:0,set_session_unread:0,update_panel_host_session:0,session_states_update:0,session_renamed:0,outboundFiltered:0,binds:0},deco:{sbiCreated:false,titleShadowOk:0,iconAsserts:0,ourWrites:0,lastIconAssertTs:0},degraded:{requireRebind:false,outboundShadow:false,titleShadow:false},lastMsgTs:0};`,
        ...
        // anchor era's try/finally. TTL sweep in hbWrite bounds orphans.
        `var OUT_REQ=Object.create(null);`,
        // TS maps for the TTL sweep (a dialog aborted by a new user message
        // never produces a response envelope; 30min TTL + panel-dispose +
        // farewell clears are the bounded leak guards).
        `var UD_TS=Object.create(null),TP_TS=Object.create(null);`,
        `var HB_LAST=0,HB_TIMER=null;`,
        `function bumpObs(k){try{var was=HB.obs[k]|0;HB.obs[k]=was+1;if(was===0)hbWrite(true)}catch(_){}}`,
        `function hbWrite(force){try{var now=Date.now();if(!force&&now-HB_LAST<30000)return;HB_LAST=now;HB.writtenTs=now;if(G.__ccsdSbi!==undefined&&G.__ccsdSbi!==null)HB.deco.sbiCreated=true;var f=RAW("node:fs");var p=IDIR+"/seam-state-"+process.pid+".json";f.writeFileSync(p+".tmp",JSON.stringify(HB));f.renameSync(p+".tmp",p);}catch(_){}}`,
        // TTL sweeps ride the throttled write (no extra timer): OUT_REQ>10min,
        // UD/TP>30min.
        `function hbSweep(now){try{for(var k in OUT_REQ){if(now-OUT_REQ[k].ts>600000)delete OUT_REQ[k]}for(var u in UD_TS){if(now-UD_TS[u]>1800000){delete UD_TS[u];if(G.__ccsdUserDialogSet)delete G.__ccsdUserDialogSet[u]}}for(var t in TP_TS){if(now-TP_TS[t]>1800000){delete TP_TS[t];if(G.__ccsdToolPermSet)delete G.__ccsdToolPermSet[t]}}}catch(_){}}`,
        // === bridge ensure + probe sentinel (three-hard-contract §6.7) ===
        // __ccsdSbi must be non-undefined at MODULE LOAD (companion:904/:952/:1077
        // probe `!== undefined` to suppress cross-window reload prompts). null is
        // probe-legit AND invisible to the IIFE's truthy singleton guards (§C
        ...
        // bundle (top-level or lazily-captured closure) resolves through the
        // wrapper. Module-local by construction: no Module._load, zero leakage
        // to other extensions in the shared EH. Mirror .resolve/.main/.cache
        // via getters so a future CC calling require.resolve/main/cache cannot
        ...
        // viewId/viewType whitelist (stable S2 string family): the panel family
        // is PREFIX-matched (tolerates suffix variants); claudePlanPreview and
        // every non-CC surface fall through untouched. The serializer shim
        // carries BOTH names — vscode 1.136 calls deserializeWebviewPanel
        // (D2-verified), older hosts called resolveWebviewPanel; restored
        // panels bypass createWebviewPanel entirely, so an un-shimmed restore
        // path = "patched but lights dead after reload" (V2 blocker #1).
        `var PANEL_RE=/^claudeVSCodePanel/,VIEW_RE=/^(claudeVSCodeSidebar|claudeVSCodeSidebarSecondary|claudeVSCodeSessionsList)$/;`,
        // v0.6.2 RESTORE-EARLY-BIND: CC session ids are dashed-lowercase-hex
        // UUIDs (e.g. 42242dfe-…). Conservative shape gate mirroring CC's own
        // D0$ format check: charset [0-9a-f-] case-insensitive, total length
        // 8..64 (rejects "remote:" sessionKeys, titles, and arbitrary junk a
        // future/older CC might put in serializer state). Seeding is OPTIMISTIC
        // — the first update_session_state/rename_tab (unseedCtx reconcile in
        // seamOnMessage) is authoritative and removes a wrong guess.
        `var SID_RE=/^[0-9a-f][0-9a-f-]{7,63}$/i;`,
        ...
        `function shimSerializer(viewType,serializer){if(!serializer||typeof serializer!=="object")return serializer;var shim=Object.create(serializer);function both(panel,state){try{armSurface(panel,viewType,"restore")}catch(_){}/*v0.6.2 RESTORE-EARLY-BIND: CC 2.1.259's serializer state carries sessionID+sessionUpdatedAt (decompiled: deserializeWebviewPanel(M,F){...let I=j?.sessionID;if(D0$(I)&&e$$(I,j?.sessionUpdatedAt))G.registerPanelSession(I,M)}) — the CC extension side knows the sid at restore INSTANT. The seam held the same state object and discarded it, leaving ctx.__ccsdSid="" until the heavyweight webview sent its first update_session_state — minutes or NEVER for a dead webview, which the §A/§H "__switching__" sentinel converted into an infinite token-SBI + favorites-SBI spinner lockout (2026-09-04 field report). Optimistic seed of the ctx armSurface just pushed (reverse CTXS scan — armSurface does not return ctx); the first real wire message stays authoritative.*/try{if(state&&typeof state==="object"){var __ss=state.sessionID;if(typeof __ss==="string"&&SID_RE.test(__ss)){var __ctx=null;for(var __i=CTXS.length-1;__i>=0;__i--){if(CTXS[__i]&&CTXS[__i].surface===panel){__ctx=CTXS[__i];break}}if(__ctx&&!__ctx.__ccsdDisposed&&__ctx.family==="panel"&&PANEL_RE.test(viewType)){seedSid(__ctx,__ss,panel)}}}}catch(_){}var fn=serializer.deserializeWebviewPanel;if(typeof fn!=="function")fn=serializer.resolveWebviewPanel;if(typeof fn==="function")return fn.call(serializer,panel,state)}shim.deserializeWebviewPanel=both;shim.resolveWebviewPanel=both;return shim}`,
        ...
        // v0.6.2 RESTORE-EARLY-BIND helper. Idempotent (no-ops when a sid is
        // already bound — the createWebviewPanel path never seeds, and a
        // second both() call for the same panel finds ctx.__ccsdSid set).
        // lastMsgTs is stamped so the outbound session_states_update sweep's
        // 10-min no-message grace counts from the SEED, not from epoch (an
        // unhydrated webview sends nothing; without the stamp, now-0>600000
        // would reap the seeded entry on CC's first outbound open-set
        // reconciliation). hostSid registration routes panel dispose +
        // farewell/panelNoLongerHosts cleanup through the existing unbindCtx.
        // Publishes __ccsdActiveSid ONLY when the restored panel is already
        // active (restarts restore the focused tab as active), mirroring the
        // v0.6.1 BUGFIX active-publish inside seamOnMessage — this is
        // precisely what prevents the §A (IIFE viewState handler) / §H
        // (per-panel tick) "__switching__" latch on the first click of a
        // restored tab.
        `function seedSid(ctx,sid,panel){try{if(!sid||!ctx||ctx.__ccsdDisposed||ctx.__ccsdSid)return;ctx.__ccsdSid=sid;ctx.__ccsdSeedSid=sid;ctx.lastMsgTs=Date.now();ensureBridges();hostSid(ctx,sid);G.__ccsdSidToPanel[sid]=panel;bumpObs("seeded");if(ctx.family==="panel"&&!ctx.__ccsdDisposed&&panel&&panel.active===true){G.__ccsdActiveSid=sid;G.__ccsdLastActiveSid=sid;try{if(G.__ccsdSbiTick)G.__ccsdSbiTick()}catch(_){}}hbWrite(false)}catch(_){}}`,
        // v0.6.2 unseed reconcile: the first authoritative sid landing
        // (update_session_state / rename_tab / update_panel_host_session
        // resolved|restore_accepted) CONFIRMS the seed (marker cleared, entry
        // kept) or REPLACES a wrong guess (seeded entry removed from the
        // bridges + registry). Without this, a serializer state whose
        // sessionID was skew (CC's own e$$ staleness gate rejected it and the
        // panel hydrated into a different session) would leave a stale
        // sidToPanel entry that the §G active-scan can pick by insertion
        // order ahead of the real sid.
        `function unseedCtx(ctx,sid){try{if(!ctx||!ctx.__ccsdSeedSid)return;var __sd=ctx.__ccsdSeedSid;ctx.__ccsdSeedSid=null;if(__sd===sid)return;if(G.__ccsdSidToPanel&&G.__ccsdSidToPanel[__sd]===ctx.panelTab)delete G.__ccsdSidToPanel[__sd];if(ctx.hosted)delete ctx.hosted[__sd];if(CTX_BY_SID[__sd]===ctx)delete CTX_BY_SID[__sd]}catch(_){}}`,
        ...
        // re-assertion calls rawSet DIRECTLY (bypassing the shadow = no loop)
        // and discriminates authorship by fsPath prefix: our icons live under
        // RESDIR, CC's under the CC extension dir — disjoint. Proxy is never
        // used anywhere (MED#7: identity landmine).
        `function shadowTitle(ctx){try{var obj=ctx.panelTab;var d=Object.getOwnPropertyDescriptor(obj,"title")||Object.getOwnPropertyDescriptor(Object.getPrototypeOf(obj),"title");if(!d||!d.set||!d.configurable)return false;var rawSet=d.set;/*v0.6.1 BUGFIX(title star-stacking): the set callback MUST be a pure passthrough. The v0.6.0 shadow recorded EVERY title write back into ctx.__ccsdTitle + the sidToTitle bridge — including OUR OWN paint write ("star X") — so the paint loop fed its own output back as the logical-title base: markers stacked ~2/s without bound and poisoned the bridge (the v0.5.46.1 bug class resurrected through the shadow path). __ccsdTitle has exactly ONE writer class now, same as the anchor era: the seam observers, from the CLEAN wire title.*/Object.defineProperty(obj,"title",{get:function(){try{return d.get.call(obj)}catch(_){return undefined}},set:function(v){try{rawSet.call(obj,v)}catch(_){}},configurable:true});return true}catch(_){return false}}`,
        ...
        // observation + heartbeat flag (never a Proxy).
        `function shadowPostMessage(ctx){try{var wv=ctx.webview;var d=Object.getOwnPropertyDescriptor(wv,"postMessage")||Object.getOwnPropertyDescriptor(Object.getPrototypeOf(wv),"postMessage");if(!d||!d.configurable||typeof d.value!=="function")return false;var rawFn=d.value;Object.defineProperty(wv,"postMessage",{value:function(msg){try{onOutbound(ctx,msg)}catch(_){}return rawFn.apply(wv,arguments)},configurable:true,writable:true});return true}catch(_){return false}}`,
        // === L2/L4/L5 armSurface — the one entry point for every captured surface ===
        `function armSurface(surface,viewType,kind){try{if(!surface||typeof viewType!=="string")return;var fam=kind==="view"?"view":"panel";if(fam==="view"&&!VIEW_RE.test(viewType))return;if(fam==="panel"&&!PANEL_RE.test(viewType))return;var ctx={panelTab:fam==="panel"?surface:null,webview:surface.webview?surface.webview:null,surface:surface,viewType:viewType,family:fam,__ccsdSid:"",__ccsdTitle:"",icon:null,__ccsdDisposed:false,lastMsgTs:0,context:{extensionPath:__dirname}};CTXS.push(ctx);HB.armed++;if(!HB_TIMER){HB_TIMER=setInterval(function(){try{hbSweep(Date.now());hbWrite(false)}catch(_){}},30000)}if(fam==="panel"){HB.surfaces.panel++;try{if(shadowTitle(ctx))HB.deco.titleShadowOk++}catch(_){}try{shadowIconPath(ctx)}catch(_){}try{if(!shadowPostMessage(ctx))HB.degraded.outboundShadow=true}catch(_){HB.degraded.outboundShadow=true}try{surface.onDidDispose(function(){try{ctx.__ccsdDisposed=true;HB.surfaces.panel=Math.max(0,HB.surfaces.panel-1);HB.armed=Math.max(0,HB.armed-1);var sid=ctx.__ccsdSid;if(sid){try{if(G.__ccsdSidToPanel&&G.__ccsdSidToPanel[sid]===ctx.panelTab)delete G.__ccsdSidToPanel[sid];if(G.__ccsdWireState)delete G.__ccsdWireState[sid]}catch(_){}}unbindCtx(ctx);hbWrite(true)}catch(_){}})}catch(_){}try{__ccsdPanelInit(ctx)}catch(_){}}else{if(viewType==="claudeVSCodeSessionsList")HB.surfaces.sessionsList=true;else HB.surfaces.sidebar=true;try{surface.onDidDispose(function(){try{ctx.__ccsdDisposed=true;HB.armed=Math.max(0,HB.armed-1);if(viewType==="claudeVSCodeSessionsList")HB.surfaces.sessionsList=false;else HB.surfaces.sidebar=false;unbindCtx(ctx);hbWrite(true)}catch(_){}})}catch(_){}}try{if(ctx.webview)ctx.webview.onDidReceiveMessage(function(m){try{HB.lastMsgTs=ctx.lastMsgTs=Date.now();seamOnMessage(ctx,m)}catch(_){}})}catch(_){}hbWrite(true)}catch(_){}}`,
        // === L3 inbound observer (design §6.3) ===
        // Envelope-final-judgment first ({type:"request",request:{type:string}});
        // per-type dispatch below carries the exact per-event semantics the
        // anchor era's replA/replB injected INLINE (sid/title stash, Layer-1a/1b
        // synchronous bridge writes, pending-set maintenance) — sourced now
        ...
        `function seamOnMessage(ctx,m){try{if(!m||typeof m!=="object"){HB.envelopeFail++;return}if(m.type==="response"){try{var rid=m.requestId;if(rid&&OUT_REQ[rid]){var e2=OUT_REQ[rid];delete OUT_REQ[rid];if(e2.sid){ensureBridges();if(e2.k==="ud"){delete G.__ccsdUserDialogSet[e2.sid];delete UD_TS[e2.sid]}else{delete G.__ccsdToolPermSet[e2.sid];delete TP_TS[e2.sid]}}}}catch(_){}return}if(m.type!=="request")return;var req=m.request;if(!req||typeof req!=="object"||typeof req.type!=="string"){HB.envelopeFail++;return}ensureBridges();var rt=req.type;if(rt==="update_session_state"){bumpObs("update_session_state");var sid=typeof req.sessionId==="string"?req.sessionId:"";if(typeof req.title==="string")ctx.__ccsdTitle=req.title;if(sid){ctx.__ccsdSid=sid;unseedCtx(ctx,sid);ensureBridges();G.__ccsdWireState[sid]=typeof req.state==="string"?req.state:"";hostSid(ctx,sid);/*v0.6.1 BUGFIX(token SBI stuck on spinner): anchor era registered onDidChangeViewState INSIDE the first handler fire — sid was already set, so the creation-time activation event never latched the "__switching__" sentinel. The seam registers it at PANEL CREATION where sid is still empty — creation activation latches the sentinel and nothing cleared it (the ported tick checks the sentinel BEFORE the map scan), so the token SBI spun forever on healthy sessions. Mirror the v0.5.35 event-handler write: sid landing on an ACTIVE panel publishes to __ccsdActiveSid/__ccsdLastActiveSid, clearing the sentinel the same millisecond.*/try{if(ctx.family==="panel"&&!ctx.__ccsdDisposed&&ctx.panelTab&&ctx.panelTab.active===true){G.__ccsdActiveSid=sid;G.__ccsdLastActiveSid=sid}}catch(_){}}if(ctx.family==="panel"&&sid&&!ctx.__ccsdDisposed){bumpObs("binds");G.__ccsdSidToPanel[sid]=ctx.panelTab;if(typeof req.title==="string")G.__ccsdSidToTitle[sid]=req.title}else if(sid&&typeof req.title==="string"){G.__ccsdSidToTitle[sid]=req.title}if(sid&&(req.panelNoLongerHosts===true||req.isFarewell===true)){try{if(ctx.family==="panel"&&G.__ccsdSidToPanel[sid]===ctx.panelTab)delete G.__ccsdSidToPanel[sid];delete G.__ccsdWireState[sid];unbindCtx(ctx)}catch(_){}}hbWrite(false);return}if(rt==="rename_tab"){bumpObs("rename_tab");var t2=typeof req.title==="string"?req.title:null;if(t2!==null)ctx.__ccsdTitle=t2;if(typeof req.sessionId==="string"&&req.sessionId){ctx.__ccsdSid=req.sessionId;unseedCtx(ctx,req.sessionId);try{if(ctx.family==="panel"&&!ctx.__ccsdDisposed&&ctx.panelTab&&ctx.panelTab.active===true){G.__ccsdActiveSid=req.sessionId;G.__ccsdLastActiveSid=req.sessionId}}catch(_){}}var sid2=ctx.__ccsdSid;if(sid2){try{if(req.hasPendingPermissions===true)G.__ccsdPendingSet[sid2]=true;else delete G.__ccsdPendingSet[sid2];if(ctx.family==="panel"&&!ctx.__ccsdDisposed){if(t2!==null)G.__ccsdSidToTitle[sid2]=t2;G.__ccsdSidToPanel[sid2]=ctx.panelTab}else if(t2!==null){G.__ccsdSidToTitle[sid2]=t2}}catch(_){}}hbWrite(false);return}if(rt==="set_session_unread"){bumpObs("set_session_unread");return}if(rt==="update_panel_host_session"){bumpObs("update_panel_host_session");try{var sid3=typeof req.sessionId==="string"?req.sessionId:"";if(!sid3&&typeof req.sessionKey==="string"&&!/^remote:/.test(req.sessionKey))sid3=req.sessionKey;if(!sid3)sid3=ctx.__ccsdSid;var st3=typeof req.state==="string"?req.state:"";if(sid3){if(ctx.family==="panel"&&!ctx.__ccsdDisposed&&(st3==="teleport_resolved"||st3==="resolved"||st3==="restore_accepted")){bumpObs("binds");G.__ccsdSidToPanel[sid3]=ctx.panelTab;ctx.__ccsdSid=sid3;unseedCtx(ctx,sid3);hostSid(ctx,sid3);try{if(!ctx.__ccsdDisposed&&ctx.panelTab&&ctx.panelTab.active===true){G.__ccsdActiveSid=sid3;G.__ccsdLastActiveSid=sid3}}catch(_){}}else if(st3==="abandoned"||st3==="restore_declined"||st3==="teleport_declined"){if(ctx.family==="panel"&&G.__ccsdSidToPanel[sid3]===ctx.panelTab)delete G.__ccsdSidToPanel[sid3];if(G.__ccsdWireState)delete G.__ccsdWireState[sid3]}}}catch(_){}return}}catch(_){}}`,
        // === L3 outbound observer (design §6.3, whitelist 4 values) ===
        ...
        // reconciliation + activeSessionId harmony (§6.5); session_renamed =
        // the rename-broadcast title source for a list-renamed session with
        // no inbound title traffic. CC 2.1.270's comm.send wraps EVERY
        // host→webview post in {type:"from-extension",message:…} — onOutbound
        // unwraps BOTH shapes (STATES.md §5 v0.6.9: the 2.1.270 rename
        // flash-revert RCA; pre-270 raw requests + tests stay supported).
        `function onOutbound(ctx,msg){try{if(!msg||typeof msg!=="object")return;if(msg.type==="from-extension"&&msg.message&&typeof msg.message==="object")msg=msg.message;/*v0.6.9: CC 2.1.270 comm.send wraps every host→webview post as {type:"from-extension",message:$}; raw {type:"request"} (pre-270 + tests) stays supported. Without this unwrap the outbound observer is a silent no-op on 270 — field-proven by heartbeat outboundFiltered:0 across 2026-09-14 (4 session_renamed sends + constant session_states_update broadcasts, zero whitelist matches).*/if(msg.type!=="request")return;var req=msg.request;if(!req||typeof req!=="object"||typeof req.type!=="string")return;var rt=req.type;if(rt!=="user_dialog_request"&&rt!=="tool_permission_request"&&rt!=="session_states_update"&&rt!=="session_renamed")return;bumpObs("outboundFiltered");ensureBridges();var sid=ctx.__ccsdSid||"";var rid=(typeof msg.requestId==="string"||typeof msg.requestId==="number")?String(msg.requestId):"";if(rt==="session_states_update"){bumpObs("session_states_update");try{var open=req.openSessionIds;var now=Date.now();if(Array.isArray(open)){var keep=Object.create(null);for(var i=0;i<open.length;i++)keep[open[i]]=true;for(var k in G.__ccsdSidToPanel){try{var c2=CTX_BY_SID[k];if(!keep[k]&&(!c2||c2.__ccsdDisposed||now-c2.lastMsgTs>600000)){delete G.__ccsdSidToPanel[k];if(G.__ccsdWireState)delete G.__ccsdWireState[k];if(CTX_BY_SID[k]===c2)delete CTX_BY_SID[k]}}catch(_){}}}}catch(_){}try{var act=req.activeSessionId;if(typeof act==="string"&&act&&G.__ccsdSidToPanel[act]){G.__ccsdActiveSid=act;G.__ccsdLastActiveSid=act}}catch(_){}return}if(rt==="session_renamed"){bumpObs("session_renamed");/*v0.6.9 4th whitelist value: handleSessionRenamed broadcasts {sessionId,title} to every comm — on 2.1.270 this fires even before renameSessionOnCli throws (the flash-revert bug), and on any CC it is the ONLY title signal for a list-renamed session with no inbound title traffic (rename_tab watches the active session only). Observer-only write (FAV.1): owner ctx via CTX_BY_SID drives the §H paint; bridge write gated to known sids so never-seen sids cannot plant orphan entries; receiving ctx is transport only; no OUT_REQ/UD_TS/TP_TS (push — no response envelope).*/try{var rsid=typeof req.sessionId==="string"?req.sessionId:"";var rtitle=typeof req.title==="string"?req.title:"";if(rsid&&rtitle){var own=CTX_BY_SID[rsid];if(own&&!own.__ccsdDisposed&&own.__ccsdTitle!==rtitle)own.__ccsdTitle=rtitle;if((own||(rsid in G.__ccsdSidToTitle))&&G.__ccsdSidToTitle[rsid]!==rtitle)G.__ccsdSidToTitle[rsid]=rtitle}}catch(_){}return}if(!sid)return;if(rt==="user_dialog_request"){G.__ccsdUserDialogSet[sid]=true;UD_TS[sid]=Date.now();if(rid)OUT_REQ[rid]={k:"ud",sid:sid,ts:Date.now()}}else{G.__ccsdToolPermSet[sid]=true;TP_TS[sid]=Date.now();if(rid)OUT_REQ[rid]={k:"tp",sid:sid,ts:Date.now()}}}catch(_){}}`,
        ...
        // panelTab/__ccsdSid/__ccsdTitle/context.extensionPath. The seam
        // supplies the identical shape; the once-guard
        // (t.__ccsdDotStarted) keeps it per-panel exactly as before. `t` is a
        // real parameter so every closure reads live values our observer
        // refreshes per event.
        `function __ccsdPanelInit(t){${""}`,
        // (the IIFE inner body is spliced as a RAW string in buildSeamPrelude's
        // tail — see the lines.push below — NOT via this template slot, so it
        // stays byte-identical to the anchor-era body.)
        `}`,
        `})();`,
    ];
    // Splice the ported IIFE inner body into __ccsdPanelInit's body slot. The
    // placeholder line above is REPLACED (not interpolated) and the body is
    // concatenated as plain text, so the inner body's own backtick / ${ }
    // sequences can never be re-interpreted — it stays byte-identical to the
    // anchor-era body. (No closing brace here: the `}` line at 2986 closes
    // __ccsdPanelInit.)
 ...
 *      /*cc-status-dot-injected:vX.Y.Z:HASH*​/
 *      ;/*ccsd2:begin:seam:vX.Y.Z*​/
 *      <prelude body>
 *      /*ccsd2:end*​/
 ...
 *  directive would demote it from directive to plain expression (a preceding
...
// for live injection but retained (with stripIifeInPlace) as the LEGACY
 ...
 *  not a failure, not retried): the seam's "prepend always succeeds" makes
 ...
 *
 *  BACKUP SEMANTICS (v0.6, G2.4): the .bak must hold the RECOVERED ORIGINAL,
 *  not a snapshot of the CURRENT on-disk bytes. On the migration path the
 *  disk still carries the injected v0.5.x bytes while `original` is the
 *  strip-recovered pristine — backupOnce (which snapshots the live file)
 *  would bake the INJECTED bytes into .bak, poisoning every future
...
function injectSeamFresh(extJs: string, original: string): void {
    ...
    const off = seamInsertionOffset(original);
    const next = original.slice(0, off) + stamped + original.slice(off);
    if (!next.includes(INJECT_MARKER) || !hasSeam(next)) {
        fail("seam insertion did not apply. No files were modified.");
    }
    log("syntax gate (node --check)…");
    assertCompiles(next, "patched extension.js (seam prelude)");
...
function patchExtensionLocked(extDir: string, extJs: string): void {
    ...
    // ② package.json type:module ④ top-level import → cc-esm-detected
    ...
    const wantSeamHash = currentSeamHash();
    if (hasSeam(src) && ver === INJECT_VERSION && diskHash === wantSeamHash) {
        // Fresh seam. Only the baked RES path may still be stale (relocation);
        // keep the anchor-era surgical literal rewrite as the cheap fix.
    ...
    if (original === null && hasSeam(src)) {
        ...
        if (stripped !== null && !isExtensionPatched(stripped)) {
            original = stripped;
            log("stripped stale seam prelude from current extension.js (no clean .bak)");
        }
    }
    if (original === null && !hasSeam(src)) {
        // v0.5.x anchor-injected install — LEGACY MIGRATION (design §5.3
        // step 5): recover via the anchor-era strip (three historical stash
        // shapes), then prepend the seam. Anchor INJECTION stays retired.
        const stripped = stripIifeInPlace(src);
        if (stripped !== null && !isExtensionPatched(stripped)) {
            original = stripped;
            log("migrating v0.5.x anchor-injected install to the v0.6 seam (legacy IIFE stripped)");
        }
    }
    if (original === null) {
        seamFail(
...
function restoreExtension(extDir: string): void {
    ...
    if (!fs.existsSync(bak)) {
        ...
        if (src !== null) {
            const seamStripped = stripSeamInPlace(src);
            if (seamStripped !== null && !isExtensionPatched(seamStripped)) {
                assertCompiles(seamStripped, "reverted extension.js (seam strip)");
                writeAtomicSync(extJs, seamStripped);
                log("reverted extension.js by stripping the v0.6 seam prelude (no .bak)");
                return;
            }
            const legacyStripped = stripIifeInPlace(src);
            if (legacyStripped !== null && !isExtensionPatched(legacyStripped)) {
                writeAtomicSync(extJs, legacyStripped);
                log("reverted extension.js by stripping the v0.5.x anchor IIFE (no .bak)");
                return;
            }
        }
        log(
...
function run(argv: string[]): void {
    ...
    if (args.includes("--emit-seam-prelude")) {
        // v0.6 test seam (undocumented, mirrors --check-iife's discipline):
        // write the seam prelude body + the stamped block to the path given
        // after the flag (or ./seam-prelude.js when omitted). hooks/
        // test-seam-runtime.mjs loads the body into a plain-node CJS wrapper
        // with a fake vscode namespace and drives L1..L7 behaviorally; the
        // spike harness embeds the stamped block into a throwaway CC copy.
        const i = args.indexOf("--emit-seam-prelude");
        const out = args[i + 1] && !args[i + 1].startsWith("--") ? args[i + 1] : "seam-prelude.js";
        ...
        const scaffold =
            ...
            "}";
        const iife = buildIIFE(RUNTIME_RES_DIR);
        let legacyInjected = scaffold.replace(ANCHOR_A, buildReplA(iife));
        legacyInjected = legacyInjected.replace(ANCHOR_B, buildReplB(iife));
        legacyInjected = legacyInjected.replace(ANCHOR_C, buildReplC());
        // The builder stamps the CURRENT version; a migration fixture must
