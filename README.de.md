...
- 🔧 **Einzeilige Installation** – `npx vscode-claude-code-status-dot` patcht die CC-Erweiterung automatisch, verdrahtet 9 Hooks, kopiert die Laufzeitdateien; idempotent und wiederholbar
...
- 📊 **SBI 4 Emoji-Bälle unten (Ball+Ziffer, Position fest)** — die linke Seite der Statusleiste (`StatusBarAlignment.Left` + 4 Slots Priorität `-9996..-9999`, nahe der sichtbaren Mitte) zeigt 4 **Emoji-Bälle mit jeweiliger Ziffer daneben** nebeneinander (v0.1.16 stellt den v0.1.14 Emoji-Ball-Stil wieder her, behält die v0.1.15 4-SBI-Struktur für feste Positionen bei): 🟢fertig / 🟡läuft / 🔵wartet / 🔴unterbrochen. Jeder Slot-Text ist `<Ball><Ziffer>` (der Ball bringt seine eigene Farbe mit — **der v0.1.15 Theme-Farbenblock + weiße Ziffer ist ENTFERNT**); Zählungen gedeckelt auf 0/1/2/3/N (>=4 zeigt N); Anzahl>0 → Ball leuchtet (🟢/🟡/🔵/🔴 vorgefärbt + Ziffer direkt daneben), Anzahl=0 → grauer Ball ⚪ + „0" (Platzhalter stimmt mit Nicht-Null-Breite überein, **Position verschiebt sich nie** — das ist der Kernvorteil von 4 SBI gegenüber v0.1.14 Single-SBI: v0.1.14s `🟢N 🟡N 🔵N 🔴N` ließ die ganze Reihe links/rechts rutschen, wenn sich Ziffernbreiten änderten; 4 SBI gibt jedem Licht seinen eigenen festen `<Ball><1-Ziffer>`-Slot, der sich nie bewegt). 🔵 = wartet auf Benutzereingabe (Permission/Question/Elicit, gespeist vom Notification-Hook-Case). Fertig vor >5 Min zählt als idle (nicht grün). v0.1.16 verwendet **4 unabhängige runtime StatusBarItem-Instanzen + Emoji-Bälle** (kein CC-package.json-Patch, kein ThemeColor-Block, die IIFE mutiert pro Slot text direkt alle 500ms) — der Emoji-Ball bringt seine eigene Farbe mit, daher ist der Render-Pfad einfacher als v0.1.15 (kein backgroundColor, kein color-Cache, kein lit/dim-Wechsel).
...
5. Schreibt **9 Hook-Ereignisse** in `~/.claude/settings.json` (mit `# cc-status-dot-managed`-Markierung, idempotent);
