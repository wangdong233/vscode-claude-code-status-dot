...
- 📊 **Faixa de 4 luzes SBI embaixo** — o lado esquerdo da barra de status (`StatusBarAlignment.Left` + prioridade muito negativa `-9999`, perto do centro visível) renderiza `🟢concluído 🟡correndo 🔵pendente 🔴interrompido` lado a lado (v0.1.14 substitui o commandCenter superior da v0.1.13, que era instável após reload/restart), cada luz limitada a 0/1/2/3/N (>=4 mostra N), tênue ⚪ em 0. 🔵 = aguardando entrada do usuário (permissão/pergunta/elicit, alimentado pelo caso de hook Notification). Concluído há >5 min conta como idle (não verde). v0.1.14 simplifica para **um único StatusBarItem em runtime** — sem parch no package.json do CC, o IIFE muta o texto diretamente a cada 500ms; a cadeia setContext→when da v0.1.13 foi removida.
