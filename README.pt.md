...
- 📊 **4 bolas SBI emoji embaixo (bola+dígito, posição fixa)** — o lado esquerdo da barra de status (`StatusBarAlignment.Left` + 4 slots prioridade `-9996..-9999`, perto do centro visível) renderiza 4 **bolas emoji com um dígito ao lado de cada uma** lado a lado (v0.1.16 restaura o estilo de bola emoji do v0.1.14, mantendo a estrutura 4-SBI do v0.1.15 para estabilidade de posição): 🟢concluído / 🟡correndo / 🔵pendente / 🔴interrompido. O texto de cada slot é `<bola><dígito>` (a bola carrega sua própria cor — **o bloco colorido por tema do v0.1.15 + dígito branco é REMOVIDO**); quantidades limitadas a 0/1/2/3/N (>=4 mostra N); quantidade>0 → bola acesa (🟢/🟡/🔵/🔴 pré-coloridas + dígito logo ao lado), quantidade=0 → bola cinza ⚪ + «0» (placeholder corresponde à largura não-zero, **posição nunca se desloca** — essa é a vantagem central do 4-SBI sobre o SBI único do v0.1.14: o `🟢N 🟡N 🔵N 🔴N` do v0.1.14 fazia a linha inteira deslizar esquerda/direita conforme as larguras dos dígitos mudavam; 4-SBI dá a cada luz seu próprio slot fixo `<bola><1-dígito>` que nunca se move). 🔵 = aguardando entrada do usuário (permissão/pergunta/elicit, alimentado pelo caso de hook Notification). Concluído há >5 min conta como idle (não verde). v0.1.16 usa **4 instâncias independentes runtime StatusBarItem + bolas emoji** (sem parch no package.json do CC, sem bloco ThemeColor, o IIFE muta o text de cada slot diretamente a cada 500ms) — a bola emoji carrega sua própria cor então o caminho de render é mais simples que v0.1.15 (sem backgroundColor, sem cache de color, sem alternância lit/dim).
