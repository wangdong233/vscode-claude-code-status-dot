...
// (v0.5.34 reverts v0.5.34's green-fallback: unknown-state must be GREY — honest
...
check(
  'IIFE.12b no-sid path renders idle (grey) fallback (v0.5.34 revert, never native orange)',
  /if\(!sid\)\{try\{p\.iconPath=ccuri[^}]*claude-logo-idle\.svg/.test(iife),
);
check(
  'IIFE.12c else read-fail path renders idle (grey) fallback (v0.5.34 revert)',
...
check(
  'IIFE.12b per-panel tick reads (j.pending===true) OR (__ccsdPendingSet[sid]===true) OR (__ccsdUserDialogSet[sid]===true) into pend (v0.5.34 three-source OR mirroring §F)',
...
check('IIFE.21c banner carries v0.5.34 stamp', /\/\*cc-status-dot-injected:v0.5.34:/.test(iife));
...
check(
  'IIFE.25b v0.5.23 §H per-panel tick reads sid.json DIRECTLY (no __ccsdAgCache — fixes §H/§F decay divergence; v0.5.34: pend carries the third __ccsdUserDialogSet OR term)',
  ...
  'v0.5.23: §H reads sid.json directly (JSON.parse(readFileSync)), NOT via §F cache. QW4 (v0.5.12 cache reuse) caused §H/§F tick desync — §H read stale cache (running, since=old) while §F read fresh (done, since=Stop) → §H decayed to idle (gray) while §F stayed done (green). Direct read ensures §H always reads latest, same as §F. v0.5.34: pend OR-chain extended with __ccsdUserDialogSet (consent/refusal coverage) mirroring §F.',
);
check(
...
// v0.5.34 ANCHOR_C sibling assertion: §F isPend must include the third OR term
...
check(
  'IIFE.29b2 v0.5.34: §F isPend includes __ccsdUserDialogSet third OR term (consent/refusal coverage mirroring §H)',
...
// IIFE.55 ANCHOR_A replA pattern).
{
  ...
  check(
    'IIFE.29c ANCHOR_B __ps keyed by this.__ccsdSid (v0.5.34: not e.request.sessionId, which is absent in rename_tab)',
  ...
  check(
    'IIFE.29c2 ANCHOR_B sid-set guarded (v0.5.34: if(e.request.sessionId) — rename_tab must NOT clear the real sid)',
...
// the literal AND the var→let fix (round-2 LOW: block-scope to the try).
{
  ...
  // v0.5.34 ANCHOR_C sentinel: askUserQuestion regression sentinel. The patch.ts
  ...
  check(
    'IIFE.29h v0.5.34 ANCHOR_C: patch.ts injects __ccsdUserDialogSet try/finally into requestUserDialog (consent/refusal sentinel; askUserQuestion implicitly via __ps)',
    ...
    'v0.5.34 ANCHOR_C / __ccsdUserDialogSet: three independent OR sources for pend (j.pending / __ps / __ccsdUserDialogSet) per R-INT-07. askUserQuestion is covered via __ps (Fact 1) — re-audit if CC reroutes it.',
  );
  check(
    'IIFE.29i v0.5.34 ANCHOR_C replC keyed by this.__ccsdSid (NOT the channelId first arg e)',
